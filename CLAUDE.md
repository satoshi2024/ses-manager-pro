...
- **Storing a file obliges you to register it in two places** — both default to the *unsafe* answer when you forget, and neither failure is visible until it bites:
  ...
  2. `FileScopeValidationService` — decides who may download a stored name. Its final fallthrough is **allow**, so an unregistered table silently becomes readable by every authenticated staff user. FR-01/FR-08 originals were in exactly that state; they are now gated on the `project-ingestion` / `bp-availability-ingestion` menu.
