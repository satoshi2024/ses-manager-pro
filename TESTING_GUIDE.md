...
- **データベース**: 高速feedbackはH2、MySQL方言・migration・並行制御はTestcontainers上のMySQL 8。
- **カバレッジ計測**: JaCoCo
...
### 3.1. テストsuite
既定は高速feedback suiteです。実MySQLとperformanceは明示profileで分離し、Docker有無による暗黙skipを無くしています。
```bash
# H2/unit/MVC（mysql/performance tagは除外）
mvn clean test

# 実MySQL/Flyway（Docker必須・ローカル逐次、CIは独立3 shard並列）
mvn clean test -Pmysql-tests

# 性能回帰（JaCoCo instrumentationなし）
mvn clean test -Pperformance-tests

# 全JUnit testを1commandで逐次実行
mvn clean test -Pfull-tests
```
...
CIとローカルは同じ3つの明示gate（fast / mysql / performance）を実行します。CIはjobを並列化し、ローカルscriptは逐次実行します。DockerまたはNode.jsが無い場合は開始前に失敗し、coverageを暗黙に縮小しません。
...
| # | gate/条件 | 内容 | 対処 |
|---|---|---|---|
| 1 | **mysql-tests** | `@Tag("mysql")` のTestcontainers testだけを逐次実行。MySQL方言・migration・lock・並行制御の正本。CIは独立3 jobへshardして並列化する。 | Docker必須。skipは失敗。`MySqlTestShardInventoryTest` がshardの漏れ・重複を検出。 |
| 2 | **fast tests / Node.js** | H2/unit/MVCに加えて `JsSyntaxCheckTest` が `static/js` 配下を `node --check` する。 | full検証ではNode.js必須。 |
| 3 | **performance-tests** | 独立H2 DBで代表データ量を計測。wall-clockを歪めるJaCoCoは無効。 | CIでは独立jobとして実行。 |
| 4 | **テスト実行順** | surefireの既定 `filesystem` はディレクトリ列挙順依存でWindowsとLinuxで変わる。fast testが1つのH2（`jdbc:h2:mem:testdb;DB_CLOSE_DELAY=-1`）を共有するため、先行testがcommitしたデータの有無で結果が変わる。 | `pom.xml` で `<runOrder>alphabetical</runOrder>` に固定済み。**自分でinsertしていない行を前提にしないこと**（`contractId = 1L` のような他test由来のIDに依存しない） |
| 5 | **TZ / ロケール / 文字コード** | 未固定だとローカル（JST・日本語）とCI（UTC・英語）で日付境界やメッセージ解決が食い違う。`LocalDate.now()` を使うtestは35ファイルある。 | `pom.xml` のsurefire `argLine` で `Asia/Tokyo` / `ja_JP` / UTF-8 に固定済み（`application.yml` の本番設定と同じ） |
