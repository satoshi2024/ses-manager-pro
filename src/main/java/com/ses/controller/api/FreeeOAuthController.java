...
public class FreeeOAuthController {
 ...
 @GetMapping("/callback") public RedirectView callback(@RequestParam String code,@RequestParam String state,HttpSession session, Authentication auth){Object expected=session.getAttribute("freee_oauth_state");session.removeAttribute("freee_oauth_state");if(expected==null||!expected.equals(state))return new RedirectView("/payroll?error=oauth"); Long uid=null; if(auth!=null&&auth.getPrincipal() instanceof com.ses.config.LoginUser u) uid=u.getSysUser().getId(); service.handleCallback(code,state,uid);return new RedirectView("/payroll?connected=1");}
