...
public interface InvoiceMapper extends BaseMapper<Invoice> {
    ...
    @Select("""
        ...
        WHERE i.id = #{id} AND i.deleted_flag = 0
          AND (
        ...
        """)
    Invoice selectForPreviewScoped(@Param("id") Long id, @Param("orgIds") java.util.List<Long> orgIds);

    /** 月次照合 (売上母集団) の組織スコープ付き一覧 (R1-P1-06 / design §5.1)。 */
    @Select("""
        <script>
        SELECT i.* FROM t_invoice i
        WHERE i.billing_month = #{month} AND i.deleted_flag = 0
          AND (
            <if test="orgIds.size() == 0">1 = 0</if>
            <if test="orgIds.size() > 0">
              EXISTS (
        ...
        """)
    List<Invoice> selectForReconciliationScoped(@Param("month") String month, @Param("orgIds") java.util.List<Long> orgIds);
