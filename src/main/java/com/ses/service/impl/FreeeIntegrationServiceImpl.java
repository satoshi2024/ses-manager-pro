...
public class FreeeIntegrationServiceImpl extends ServiceImpl<FreeeConnectionMapper, FreeeConnection> implements FreeeIntegrationService {
    ...
    public String authorizationUrl(String state) {
        return UriComponentsBuilder.fromUriString(apiBase + "/oauth/authorize")
                .queryParam("client_id", clientId)
                .queryParam("redirect_uri", redirectUri)
                .queryParam("response_type", "code")
                .queryParam("scope", "read:hr employees:read payrolls:read")
    ...
    public void handleCallback(String code, String state, Long userId) {
        ...
        if (n == null || n.path("access_token").asText().isBlank()) {
            throw BusinessException.of("error.payroll.oauthFailed");
        }
        ...
        c.setAccessTokenEncrypted(encrypt(n.path("access_token").asText()));
        c.setRefreshTokenEncrypted(encrypt(n.path("refresh_token").asText()));
        c.setTokenExpiresAt(LocalDateTime.now().plusSeconds(n.path("expires_in").asLong(3600)));
    ...
    public boolean connected() {
        return connectionMapper.selectCount(new LambdaQueryWrapper<>()) > 0;
    }

    @Override
    public void disconnect() {
        connectionMapper.delete(new LambdaQueryWrapper<>());
    }
