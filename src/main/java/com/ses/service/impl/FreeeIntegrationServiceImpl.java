...
public class FreeeIntegrationServiceImpl extends ServiceImpl<FreeeConnectionMapper, FreeeConnection> implements FreeeIntegrationService {
    ...
    private JsonNode get(String path){FreeeConnection c=connectionMapper.selectOne(new LambdaQueryWrapper<FreeeConnection>().orderByDesc(FreeeConnection::getId).last("LIMIT 1")); if(c==null)throw BusinessException.of("error.payroll.notConnected"); HttpHeaders h=new HttpHeaders(); h.setBearerAuth(decrypt(c.getAccessTokenEncrypted())); h.setAccept(List.of(MediaType.APPLICATION_JSON)); try{return restTemplate.exchange(apiBase+path,HttpMethod.GET,new HttpEntity<>(h),JsonNode.class).getBody();}catch(Exception ex){throw BusinessException.of("error.payroll.providerUnavailable");}}
