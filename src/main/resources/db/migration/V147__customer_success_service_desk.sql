...
CREATE TABLE IF NOT EXISTS t_service_sla_clock (
    ...
    created_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_sla_clock_request_round (service_request_id, round_no),
...
CREATE TABLE IF NOT EXISTS t_service_comment (
    ...
    author_id           BIGINT NOT NULL DEFAULT 0 COMMENT '投稿者ID',
    author_name         VARCHAR(100) NOT NULL COMMENT '投稿者名',
    visibility          VARCHAR(20) NOT NULL DEFAULT 'PORTAL_VISIBLE' COMMENT 'PORTAL_VISIBLE, INTERNAL',
    comment_text        TEXT NOT NULL COMMENT '本文',
    created_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='リクエストコメント';
...
CREATE TABLE IF NOT EXISTS t_service_state_event (
    ...
    actor_id            BIGINT NOT NULL DEFAULT 0 COMMENT '実行者ID',
...
-- 11. メニューマスタへのサービスデスク・カスタマーサクセス追加
INSERT INTO m_menu (menu_key, menu_name, path_prefix, api_prefix, sort_order)
VALUES 
('service-desk', 'サービスデスク', '/service-desk', '/api/service-desk', 11),
('customer-success', 'カスタマーサクセス', '/customer-success', '/api/customer-success', 12)
...
WHERE m.menu_key IN ('service-desk', 'customer-success');
