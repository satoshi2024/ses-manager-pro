...
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @sql = (SELECT IF(COUNT(*) = 0,
  'ALTER TABLE t_external_account_reference ADD CONSTRAINT ck_ext_revoke_status_attribution CHECK (status IS NOT NULL AND (status <> ''REVOKED'' OR (revoke_confirmed_at IS NOT NULL AND actor_type IS NOT NULL AND confirmation_source IS NOT NULL)))',
