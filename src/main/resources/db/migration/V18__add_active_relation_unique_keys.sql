...
ALTER TABLE t_bp_payment
  ADD COLUMN active_work_record_id BIGINT
    GENERATED ALWAYS AS (IF(deleted_flag = 0, work_record_id, CAST(NULL AS SIGNED))) VIRTUAL;

-- t_bp_payment: add generated column for layer_order
ALTER TABLE t_bp_payment
  ADD COLUMN active_layer_order INT
    GENERATED ALWAYS AS (IF(deleted_flag = 0, layer_order, CAST(NULL AS SIGNED))) VIRTUAL;
...
ALTER TABLE t_contract
  ADD COLUMN active_proposal_id BIGINT
    GENERATED ALWAYS AS (IF(deleted_flag = 0, proposal_id, CAST(NULL AS SIGNED))) VIRTUAL;

-- t_contract: add generated column for renewed_from_contract_id
ALTER TABLE t_contract
  ADD COLUMN active_renewed_from_contract_id BIGINT
    GENERATED ALWAYS AS (IF(deleted_flag = 0, renewed_from_contract_id, CAST(NULL AS SIGNED))) VIRTUAL;
