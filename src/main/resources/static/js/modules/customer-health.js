/**
 * 顧客ヘルススコア・CSダッシュボード JS (100点減点モデル)
 */
(function (window, $) {
    'use strict';
    ...
    function generateSnapshot() {
        Swal.fire({
            title: '月次スナップショット作成',
            text: '全顧客の最新ヘルススコアを計算し、今月のスナップショットを記録しますか？',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.code === 200) {
                            if (window.Toast) {
                                window.Toast.success('スナップショットを作成しました');
                            } else {
                                Swal.fire('完了', 'スナップショットを作成しました', 'success');
                            }
                            loadHealthData();
                        } else {
                    ...
                    error: function (xhr) {
                        const msg = xhr.responseJSON ? xhr.responseJSON.message : '通信エラーが発生しました';
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.code === 200) {
                    ...
                    $('#qbrForm')[0].reset();
                    if (window.Toast) {
                        window.Toast.success('定例会(QBR)記録を保存しました');
                    } else {
                        Swal.fire('保存完了', '定例会(QBR)記録を保存しました', 'success');
                    }
                    loadHealthData();
                } else {
