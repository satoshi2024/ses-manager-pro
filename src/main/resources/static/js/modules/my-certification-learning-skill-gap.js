...
(function () {
    ...
    window.showMyCertification = async function (id) { try { const item = await SES.api.get(base + '/certifications/' + encodeURIComponent(id)); const record = item.record || {}; Swal.fire({ title: esc(record.certificationDisplayName || '資格詳細'), html: '<p>状態: ' + esc(record.recordState || '-') + '</p><p>取得日: ' + esc(record.acquiredOn || '-') + '</p><p>期限: ' + esc(record.expiresOn || '-') + '</p><p>証憑: ' + ((item.evidences || []).length) + '件</p>', confirmButtonText: '閉じる' }); } catch (e) { showError(e); } };
