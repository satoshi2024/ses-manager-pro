        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.code === 200) {
                    ...
                    SES.toast.error(res.message || 'データ取得に失敗しました');
                }
            },
            error: function () {
...
function initDetailPage(requestId) {
    ...
    function loadDetail(id) {
        $.ajax({
            ...
            error: function () {
                SES.toast.error('詳細データの取得に失敗しました');
            }
        });
    }
    ...
    function renderDetail(data) {
        ...
        $('#ownerUserNameText').text(data.ownerUserName || '未割当');
    ...
    function renderActionButtons(data) {
        ...
        $btns.find('.btn-status').on('click', function () {
            ...
            handleStatusChange(data.id, nextStatus, data.version);
        });
    }

    function handleStatusChange(id, toStatus, version) {
        let reasonPrompt = 'ステータスを変更しますか？';
        if (toStatus === 'RESOLVED') reasonPrompt = '解決理由や対応結果を入力してください';
        if (toStatus === 'REOPENED') reasonPrompt = '再オープンの理由を入力してください';
        if (toStatus === 'WAITING_CUSTOMER') reasonPrompt = '確認待ちの理由・項目を入力してください';

        Swal.fire({
            title: `ステータス変更: ${toStatus}`,
            input: 'text',
            inputLabel: reasonPrompt,
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.code === 200) {
                            ...
                            SES.toast.error(res.message || '更新に失敗しました');
                        }
                    },
                    error: function (xhr) {
    ...
    function submitComment(requestId) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.code === 200) {
                    SES.toast.success('コメントを投稿しました');
                    $('#commentTextInput').val('');
                    loadDetail(requestId);
                } else {
                    SES.toast.error(res.message || '投稿に失敗しました');
                }
            },
            error: function () {
                $('#submitCommentBtn').prop('disabled', false);
                SES.toast.error('投稿中にエラーが発生しました');
            }
        });
    }
}
