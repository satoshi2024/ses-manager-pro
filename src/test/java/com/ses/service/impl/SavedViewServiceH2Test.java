...
public class SavedViewServiceH2Test {
    ...
    void testSavedViewAllowlistValidation() {
        ...
        invalidView.setPageKey("engineer_list");
        invalidView.setName("不正なビュー");
        invalidView.setFilterJson("{\"passwordHash\": \"1\"}");

        assertThrows(BusinessException.class, () -> savedViewService.createView(invalidView, 100L, false));

        // 許可されているフィールド名で 200 (正常作成)
        SavedView validView = new SavedView();
        validView.setPageKey("engineer_list");
        validView.setName("正常なビュー");
        validView.setFilterJson("{\"prefecture\": \"Tokyo\"}");
        validView.setColumnsJson("[\"fullName\", \"status\"]");

        SavedView created = savedViewService.createView(validView, 100L, false);
    ...
    void testPersonalAndSharedViews() {
        ...
        userAView.setPageKey("engineer_list");
        userAView.setName("ユーザーAのビュー");
        userAView.setFilterJson("{\"status\":\"稼動中\"}");
        ...
        sharedView.setPageKey("engineer_list");
        ...
        List<SavedView> viewsB = savedViewService.getViewsForUserAndPage(200L, "engineer_list");
        assertEquals(1, viewsB.size());
        assertEquals("全社標準ビュー", viewsB.get(0).getName());

        // 4. ユーザー A (100L) が参照した場合、個人ビュー＋共有ビューの両方が取得される
        List<SavedView> viewsA = savedViewService.getViewsForUserAndPage(100L, "engineer_list");
    ...
    void testAdminCannotUpdateOtherUserPersonalView() {
        ...
        personalView.setPageKey("contract_list");
        personalView.setName("ユーザーA専用ビュー");
        personalView.setFilterJson("{\"status\":\"有効\"}");
        ...
        updateAttempt.setPageKey("contract_list");
        updateAttempt.setName("改ざん試行");
        updateAttempt.setFilterJson("{\"status\":\"有効\"}");
