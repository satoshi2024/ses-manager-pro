...
public class QuotationServiceImplTest {
    ...
    @Test
    void update_afterClosedOnlyRemarks() {
        Quotation current = new Quotation();
        current.setId(1L);
        current.setStatus("受注");
        when(quotationMapper.selectById(1L)).thenReturn(current);
        when(quotationMapper.updateById(any(Quotation.class))).thenReturn(1);

        Quotation edit = valid();
        edit.setId(1L);
        edit.setRemarks("追記");
        edit.setUnitPrice(new BigDecimal("999999")); // 無視されるべき
        ...
        assertEquals("追記", cap.getValue().getRemarks());
        assertNull(cap.getValue().getUnitPrice(), "受注後は備考以外は反映されない");
    }
