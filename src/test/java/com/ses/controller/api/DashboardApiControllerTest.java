...
class DashboardApiControllerTest {
    ...
    @Test
    @WithMockUser
    void testGetUtilizationForecast() throws Exception {
        UtilizationForecastDto dto = UtilizationForecastDto.builder()
                .monthlyForecasts(Collections.emptyList())
                .rolloffEngineers(Collections.emptyList())
                .build();
        when(utilizationForecastService.getForecast(3)).thenReturn(dto);

        mockMvc.perform(get("/api/dashboard/utilization-forecast").param("months", "3"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
}
