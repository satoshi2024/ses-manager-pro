...
class IntegrationHubB2MigrationContractTest {
    @Test
    void V141はinboundReplayをpayloadと独立したmetadataledgerへする() throws IOException {
        String sql;
        try (var stream = getClass().getResourceAsStream(
                "/db/migration/V141__integration_hub_public_api_b2_inbound_replay.sql")) {
    ...
    @Test
    void V142はproviderとresourceとopaque管理referenceを固定する() throws IOException {
        String sql;
        try (var stream = getClass().getResourceAsStream(
                "/db/migration/V142__integration_hub_public_api_b2_binding_refs.sql")) {
    ...
    @Test
    void V143はinboundProcessingLease列とstaleRecovery索引を追加する() throws IOException {
        String sql;
        try (var stream = getClass().getResourceAsStream(
                "/db/migration/V143__integration_hub_inbound_processing_lease.sql")) {
            if (stream == null) throw new IOException("missing V143 migration");
            sql = new String(stream.readAllBytes(), StandardCharsets.UTF_8).toLowerCase();
        }
