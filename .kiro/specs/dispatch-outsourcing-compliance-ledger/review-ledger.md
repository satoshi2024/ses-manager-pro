...
T060からT061へ進む条件は、R10 Round 2で確認済みのR1-P1-01に加え、R1-P1-02の有効な社内承認event（active assignment、承認権限、actor、承認status、承認日時、mapping version/hash、公式source版）が対象blob/hashと一致してrepoの証跡として確認され、tasks.mdのT060が `[x]` へ戻ること。R10がP1-02をVERIFIED_CLOSEDするまでT061/V82/production変更を開始しない。T061開始時にはmerge済み `db/migration` のlatestを再確認する。
