...
- [ ] M. 法務受入/回帰（T066実装・L4全量実行済み・G2 gate未達のためPASS条件未達）
  ...
  - **テスト要件**: R10がdecision deltaを`ACCEPTED_FOR_IMPLEMENTATION`とした後、L1〜L3で
    ...
    `G2-IDP-01..14`、`G2-LIFE-01..10`を実行する。
  ...
  - **Demo**: 法務fixture3契約の台帳とfindingを照合し、既存4 ruleの出力不変を提示する。
    ...
    resolved workplace ID、render_input_hash、worker snapshotなし時のID/hash同時NULL・worker項目省略・生成継続、role別403、4帳票、master/config/profile/worker変更後のbytes/hash不変、SUPERSEDED後再downloadを確認する。
