...
- [x] A1. 契約compliance profile/UI（R10 Round 13でT063 A1 PASS・R12-P1-01/P2-01 VERIFIED_CLOSED）
...
- [x] B2. deadline/リスク運用（R10 Round 18でT065 B2 PASS・R18-P1-01 fix済み）
...
- [ ] M. 法務受入/回帰（T066実装・L4全量実行済み・G2 gate未達のためPASS条件未達）
  ...
  - **テスト要件**: R10がdecision deltaを`ACCEPTED_FOR_IMPLEMENTATION`とした後、L1〜L3で
    ...
    `G2-ACT-01..06`、`G2-DEL-01..16`、`G2-SEC-01..18`、`G2-MIG-01..12`、
  ...
  - **Demo**: 法務fixture3契約の台帳とfindingを照合し、既存4 ruleの出力不変を提示する。
    ...
    resolved workplace ID、render_input_hash、worker snapshotなし時のID/hash同時NULL・worker項目省略・生成継続、時刻を跨ぐ異key再生成の業務一意性、legacy generation_state=NULLのdownload、future_slot成功解放・失敗rollback、role別403、4帳票、master/config/profile/worker変更後のbytes/hash不変、SUPERSEDED後再downloadを確認する。
