...
| `lifecycle-status-consistency` | 未着手（2026-07-17 第3次業務ロジック横断調査の是正spec） | A→C、B並行 | 営業/ユーザーライフサイクル(`UserApiController`/`EngineerSales*`)・通知(`NotificationGenerateService`)・要員状態/勤怠(`Engineer*`/`ContractServiceImpl.removeById`/`WorkRecordServiceImpl.saveHours`) | `.kiro/specs/lifecycle-status-consistency/tasks.md に従い、割り当てられたレーンだけを実装してください。完了したタスクは - [x] にチェックしてください。` |
