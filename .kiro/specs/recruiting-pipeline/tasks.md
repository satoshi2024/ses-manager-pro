...
- [x] F1. DDL・エンティティ・Mapper・メニュー権限基盤
  ...
  - **実装メモ**: `db/migration/V15__add_webhook_config.sql`が既に存在するため、実際のDDLは`V16__create_candidate_tables.sql`として追加した(design.mdの`V15`表記は文書作成当時の想定バージョン)。

- [x] A. 候補者CRUD・ステージ管理API 【並行可・担当ファイル: entity/Candidate*, mapper/Candidate*, service/CandidateService*, controller/api/CandidateApiController, CandidateServiceImplTest】
  ...
  - **実装メモ**: `mvn test`で`CandidateApiControllerTest`の権限系統(HR/営業/管理者=200、マネージャー=403)を含め確認済み。

- [x] B. 画面(一覧・詳細・ステージタイムライン) 【並行可・担当ファイル: candidate/list.html, candidate/detail.html, static/js/modules/candidate.js】
  ...
  - **実装メモ**: 手動UI確認は環境上未実施(バックエンド経由の自動テストで代替)。テンプレート/JSはコンパイル・診断エラーなしを確認済み。
...
- [x] M1. 入社→エンジニア変換連携
  ...
  - **実装メモ**: `engineer/form.html`は本リポジトリに存在せず(要員新規作成は`engineer/list.html`内のBootstrapモーダル`#engineerModal`で行う既存設計)、代わりに`engineer/list.html`+`engineer.js`にクエリパラメータ(`prefillCandidateId`/`prefillName`/`prefillSkillSummary`)でのモーダル初期値補完を実装した。`CandidateServiceImplTest#linkConvertedEngineer_convertedEngineerIdが設定される`で`convertedEngineerId`設定を確認済み。
