...
| HFP-02-REV-008 | P2 | HFP-02-03,05 / HFP-02-AC-09-02 | `service/cloudsign/CloudSignRateLimiter.java`（JVM内 Deque） | budget（既定500/min）がJVM単位で、複数instance（ShedLock前提の構成）では合算が公式800 request/token/minを超え得る。共通limiterの要件に対して分散同期が無い | 429誘発の可能性。実害はbackoffで緩和されるが公式上限を守る設計になっていない | instance数想定を design/運用に記録して budget=800/N に設定するか、分散limiter化（既存基盤の範囲で） | VERIFIED_CLOSED（設計判断をReviewerが追認: 公式上限は同一 access token 単位であり、token はJVM毎single-flightのためJVM単位budget≤500が契約内） | 独立Review AI / 2026-08-14 |
...
## 7.2 Round 2 独立Review結果（fix delta `89ff96e6..0afd2af3`）

| Finding ID | Round 2 判定 | 根拠（Reviewer独立確認） |
|---|---|---|
| REV-001 | VERIFIED_CLOSED | JSが `d.contractNo`（detail DTO）を使用し、queueSendの契約番号検証と一致することをコードで確認。full suite 1981/0/0/0 |
| REV-002 | VERIFIED_CLOSED | `POLL_STATES`=SENTのみ（pollがRECONCILIATION行をGETしないtest確認）。manual syncは file ID＋宛先email 一致証明時のみ復旧（4 test: 自動遷移しない/一致復旧/宛先不一致/ファイル不一致）。design §6.2の専用操作要件を満たす。reviewer監査はApiAuditFilterのPOST /sync監査で担保 |
| REV-003 | VERIFIED_CLOSED | queueSend先頭で `enabled=false` を `cloudsignNotConfigured` で拒否（message key実在確認、`casQueue` 非呼出しtest） |
| REV-004 | **OPEN（残課題）** | TRANSIENT分岐は追加されたが、`retryWait` の attempt 上限が `safeVersion(working) >= maxAttempts(5)` で判定されるため、後段工程（version≥5で到達する UPLOADING以降、特にSENDING preflight）では最初の1回の一時障害で即 `FAILED_FINAL:ATTEMPT_LIMIT:TRANSIENT:*` になる。REV-004の目的（bounded retry）が後段で機能しない。dispatch_attempt_count（casClaim/casGetBackoffが増分）を使うか、versionとattemptを分離して再判定。再現: SENDING行（version≥8）でpreflight GETをSocketTimeout→handleApiFailure→retryWait→即FAILED_FINAL。testはversion低いCREATING起点のみで検出不達 |
| REV-005 | VERIFIED_CLOSED | 25同時test実在・PASS（25受付・operation 1件・provider create 0） |
| REV-006 | VERIFIED_CLOSED | signed/cert download失敗×2・ledger例外の3 test実在・PASS。finding code記録・batch継続・call count=1をassert。FND-006のclose evidence（error state test）充足 |
| REV-007 | VERIFIED_CLOSED | `detailOf` が顧客 `companyName` を解決し modal に「宛先会社」表示。JS syntax/page test green |
| REV-008 | VERIFIED_CLOSED | 設計判断を追認: 公式429は「同一 access token 800/min」であり、tokenはJVM毎single-flight（共有tokenなし）のためJVM単位budget≤500は契約内。ledger §7.1記録済み |
| REV-009 | **OPEN（merge時）** | 経緯記録は確認。中央 `execution-ledger.md` への coordinator 転記・承認は未実施のまま（本Reviewerは中央ledgerを変更しない）。merge前の転記を要求 |
| REV-010 | VERIFIED_CLOSED | no-op判定に `existingArchiveId != null` を追加し、未登録時は台帳登録へ進む。test実在・PASS（hash不変・t_document 1件） |
| REV-011 | VERIFIED_CLOSED | 設計範囲内と追認（queue時hash/magic/EOF検証済み・size上限はworkerで外部呼出し前に拒否） |
| REV-012 | **OPEN（claimと差分不一致）** | 対応表は「controller testへ追加/RESOLVED」と記載するが、fix delta にマネージャーの新規test methodが存在しない（controller testは8件のまま）。NOTEのためblockはしないが、claim修正またはtest追加を求める |
| REV-013 | VERIFIED_CLOSED | Dispatch/Sync/Artifact統合testは @BeforeEach で契約・テンプレートを自前insert（`contractMapper.insert` 確認）。engineer/project/customer IDは実参照のない値。Round 1の指摘は実態と異なっていたため撤回 |
| REV-014 | **OPEN（merge時）** | origin/main 17 commit未取り込み。merge時の衝突再確認と採番判断のcoordinator記録を要求 |

### Round 2 検証実行（独立Review AI）
| 日時 | command | tests | failure | error | skip | 結果 |
|---|---|---:|---:|---:|---:|---|
...
|---|---|---|---|---|---|
| REV-004 | P2 | retry上限をversionからretry回数(dispatch_attempt_count)へ分離。casClaimはattemptを増やさず、casRetryWait/backoffのみ増加させる（後段工程の最初の一時障害でFAILED_FINAL化しない） | ContractDocumentMapper.casClaim/casRetryWait・CloudSignDispatchService.retryWait(safeAttempts) | CloudSignDispatchIntegrationTest#SENDING起点の一時障害はFAILED_FINALにならずbackoffする / #retry上限到達時はFAILED_FINALになる・ContractDocumentDispatchStateTest#retryWaitはretry回数を増やし次回試行時刻を設定する | FIXED |
| REV-012 | NOTE | マネージャーroleの明示test追加 | ContractDocumentApiControllerTest#マネージャーも送信queueとsyncを実行できる | 9/9 PASS | FIXED |
| REV-013 | NOTE | Round 1指摘を撤回（各testが自前insert。他クラス行依存なし） | - | - | WITHDRAWN |
| REV-009/014 | - | merge時対応（coordinator転記・origin/main取り込み・V109採番再確認） | - | - | OPEN(merge時) |

### Round 2 fix delta 検証
| 日時 | command | tests | failure | error | skip | 結果 |
|---|---|---:|---:|---:|---:|---|
