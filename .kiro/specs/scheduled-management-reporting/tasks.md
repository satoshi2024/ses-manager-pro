...
- [x] B1: PDF/XLSX/CSV と DocumentService 登録。同一snapshot、hash/version/CLEAN、7年保持、scope/access auditを実装する。
  ...
  - Evidence: `ReportDocumentServiceImpl`、`ReportDocumentArtifact`、document API。XLSX/CSVのformula injection対策と生成サイズ上限を含み、同じsnapshotの`valueJson`を画面/CSV/XLSXで扱うcontract testを持つ。
...
- [x] B2: schedule、outbox、link/re-auth、retry、DLQ/manual replay。アプリ内通知＋期限付きlink、生成/download scope、再認証を実装する。
  ...
  - Evidence: `ReportScheduleServiceImpl`、`ReportScheduleMapper`、`ManagementReportScheduler`、`ReportDeliveryServiceImpl`、delivery API、`NotificationOutboxDispatcher`、`FileScopeValidationService`。保存済みscopeとcronをscheduleへ固定し、tokenはhashのみ保存、期限7日、download前password再認証10分、権限・組織scopeを再検証、deliveryを`ENQUEUED`からoutbox dispatch結果へ同期し、retry/DLQ/manual replayを実装する。MANAGEMENT_REPORTは汎用Document downloadを拒否し、ENQUEUED/PROCESSING中のdelivery再実行は新outboxを作らない。
...
- [x] M: contract test、月末境界、desktop/390px、restore、配布障害訓練、base/head 証拠。required gatesをskip 0で実施する。
  ...
  - Evidence: `ReportSnapshotServiceImplTest` 6/6、`ReportRecipientPreviewServiceImplTest` 2/2、`ReportDeliveryServiceImplTest` 7/7、`FileScopeValidationServiceTest` 6/6、`NotificationOutboxDispatcherTest` 5/5、`NotificationServiceImplTest` 9/9、`ManagementReportSchedulerTest` 3/3、`ReportScheduleServiceImplTest` 3/3、`ReportDocumentServiceImplTest` 3/3、`CashFlowForecastServiceTest` 14/14、`ActionPermissionResolverTest` 11/11、`AllMappersSchemaSweepTest` 175/175、`FlywaySelfServiceSchemaSmokeTest` 3/3、合同targeted gate 247/247、`MobileResponsiveLayoutTest` 29/29、MySQL V112/V113 smoke 5/5、performance 1/1 (p95=64ms)。backup unit各suiteはfailures=0、backup integrationはskip 0でSUCCESS（NF-10 report restore contract、RPO/RTO、restore、cutover rollback、secret scanを含む）。
