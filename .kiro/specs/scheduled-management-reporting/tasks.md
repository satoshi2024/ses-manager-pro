...
- [x] F1: template/version/schedule/run/snapshot/delivery の DDL。最新migration+1、V1/H2同期、shape test、7年保持を実装する。
  - Evidence: `V112__scheduled_management_reporting.sql`、`V113__scheduled_management_report_audit.sql`、`V114__report_schedule_processing_lease.sql`、`schema-scheduled-management-reporting-h2.sql`、7 entity/mapper、test schema location。
...
- [x] F2: explicit system principal/scope の snapshot orchestration。管理者/マネージャーscope、速報/確定、retry不変性、partial/failed停止を実装する。
  ...
  - Evidence: `ReportSnapshotServiceImpl`、`ReportRecipientPreviewServiceImpl`、`ReportSectionAttempt`。run/section一意キー、通常retry再利用、生成直前のcurrent entity scope再解決、明示再生成時の新version、新規run、部分失敗時配布停止状態、section attempt append-only監査を実装。
...
- [x] A1: template/preview/run UI。管理者有効化、recipient preview、actual/forecast、dataAsOf/freshness、Asia/Tokyoを表示する。
  ...
  - Evidence: `ManagementReportApiController`、`ManagementReportPageController`、`management-reports/index.html`、`management-reports.js`、`ReportTemplateServiceImpl`のdraft version作成/編集/公開、`SecurityConfig`の静的role境界。
...
- [x] B1: PDF/XLSX/CSV と DocumentService 登録。同一snapshot、hash/version/CLEAN、7年保持、scope/access auditを実装する。
  ...
  - Evidence: `ReportDocumentServiceImpl`、`ReportDocumentArtifact`、document API。XLSX/CSVのformula injection対策と生成サイズ上限を含み、正本DTO→画面→PDF/XLSX/CSVを同一metric fixtureで比較するcontract testを持つ。
...
- [x] B2: schedule、outbox、link/re-auth、retry、DLQ/manual replay。アプリ内通知＋期限付きlink、生成/download scope、再認証を実装する。
  ...
  - Evidence: `ReportScheduleServiceImpl`、`ReportScheduleMapper`、`ManagementReportScheduler`、`ReportDeliveryServiceImpl`、delivery API、`NotificationOutboxDispatcher`、`FileScopeValidationService`。schedule作成時scopeは監査境界として保存し、生成直前に明示principalのcurrent entity scopeを再解決する。tokenはhashのみ保存、期限7日、download前password再認証10分、権限・組織scopeを再検証、deliveryを`ENQUEUED`からoutbox dispatch結果へ同期し、既存outbox再利用のretry/DLQ/manual replayを実装する。MANAGEMENT_REPORTは汎用Document downloadを拒否し、非`RETRY`状態のdelivery retryは新outboxを作らない。
...
- [x] M: contract test、月末境界、desktop/390px、restore、配布障害訓練、base/head 証拠。required gatesをskip 0で実施する。
  ...
  - Evidence: `ReportSnapshotServiceImplTest` 8/8、`ReportRecipientPreviewServiceImplTest` 2/2、`ReportDeliveryServiceImplTest` 16/16、`FileScopeValidationServiceTest` 6/6、`NotificationOutboxDispatcherTest` 5/5、`NotificationServiceImplTest` 9/9、`ManagementReportSchedulerTest` 4/4、`ReportScheduleServiceImplTest` 3/3、`ReportTemplateServiceImplTest` 2/2、`ReportDocumentServiceImplTest` 3/3、`ReportMetricContractTest` 2/2、`CashFlowForecastServiceTest` 14/14、`ActionPermissionResolverTest` 11/11、`AllMappersSchemaSweepTest` 175/175、`FlywaySelfServiceSchemaSmokeTest` 3/3、`FlywayV110AdminBoundaryUpgradeSmokeTest`（latest=114）、`MobileResponsiveLayoutTest` 30/30、MySQL V112–V114 smoke 5/5、performance 1/1 (p95=64ms)。backup unit各suiteはfailures=0、backup integrationはskip 0でSUCCESS（NF-10 report restore contract、RPO/RTO、restore、cutover rollback、secret scanを含む）。
  - Boundary/incident evidence: 月末 `2026-08-01..31`、Asia/Tokyo、速報の `GENERATED_AT` cutoff、確定の月次締め拒否、同一run retry再利用、明示regenerationの親run・snapshot version記録、scope変更download拒否、delivery attempt 5のDLQ、manual replay、期限切れlink拒否、recipient preview hash、processing lease stale reclaimを検証した。schedule初回next_run_atは保存cronの次回発火時刻を使う。
  - Demo: `/management-reports` のdesktop/390px DOM/responsive検証は `MobileResponsiveLayoutTest` 30/30。専用browser testはJava 21のloopback制約でTomcat起動前に再現失敗し、スクリーンショットは生成されなかったため、Mの環境制約として記録する。document restoreはbackup integrationのrestore/validate-restoreとcutover rollbackで証明し、NF-10固有smokeでrun/snapshot/section attempt/document version/outbox/deliveryの復元整合性を確認する。V114を含むfresh/legacy MySQL migration smokeは5/5。
