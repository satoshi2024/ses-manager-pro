...
## Current authoritative REVIEW PACKET — code baseline Head 68fbbba / 現行Review evidence commit 646dbda（2026-08-05）

**判定:** **REVIEW（独立再Review待ち）**。S07の技術・機能gateは独立Reviewで全て確認済み（R4-P1-01・B1/T046・M/T047は`[x]`、R4-REVIEW-01/02/03・R4-P1-01はVERIFIED_CLOSED（R4-REVIEW-04は文書整合の独立再Review待ち））。本Packetの文書整合（Head分離・AC trace同期・中央ledger row 7〜9の統一）を独立再Reviewで確認後、S07 PASS・S09 READY・Wave 2解放へ一括遷移する。それまではS09=`NOT READY`、Wave 2=`未解放`を維持する。

- **code baseline Head / Review evidence commit:** Review Base=`5d228d211d0d752833fe3424a3b8aa4b40096733`。**code baseline Head**=`68fbbba4dff8255b3a745ce61e73e686a78bef3e`（Base→Head **23 commits / 219 paths**。`68fbbba`時点の`HEAD = origin/main = origin/HEAD`一致は**当時値**）。**初回Review evidence commit**=`2978461be1fd36334a00a97fabe37f5613e374a4`（Base→commit **24 commits / 272 paths**、browser evidence・runbook/test修正・Packet文書を含む。**履歴**）。**現行Review evidence/result commit**=`646dbdafb3c6b77ec0e3b7bb581392f50be53491`（Base→commit **27 commits / 274 paths**、seed修正＋browser evidence再生成）。いずれもcode baseline Headと分離して管理する。直前の`6680e7d81c7842262a2fd07c57fb9942e80573ce`（および旧`1e8a224`）は履歴としてのみ扱う。最終確認時のworktreeは**clean**。文書同期commitは`git log -1 -- <path>`で解決する（e88351d時点の基準はBase→**26 commits / 274 paths / +16873/-342**）。
- **scope:** Base→code baseline Headは**23 commits / 219 files / +11639/-337**（=219 paths）。219 paths（#001〜#212＋`68fbbba`で追加された7 unique paths #213〜#219）はすべてcode baseline Headにcommit済みの範囲である。7 unique paths（`v79_1-fk-actions-forward-repair.sql`、`SchedulerLockConfig.java`、`application.yml`、`SchedulerLockH2IntegrationTest.java`、`FlywayV79_1RepairSmokeTest.java`、`OperationalBoundaryMySqlIntegrationTest.java`、`WebhookNotifierLoopbackIntegrationTest.java`）はmanifest §1.1/§2.10へ帰属済みである。初回Review evidence commit `2978461`はcode baseline Headに+53 files（browser evidence・runbook/test修正・Packet文書）を加えた**272 paths**、現行Review evidence/result commit `646dbda`は**27 commits / 274 paths**。
- **Packet文書commit / provenance:** **Packet文書は本流`main`の履歴としてcommitされる（専用の独立Packetブランチ/リポジトリは持たない）。文書自身のcommit（例: `d13e726`は文書同期commit）は`git log -1 -- <path>`で解決するprovenanceとして記載し、code baseline Head（`68fbbba`）や現行Review evidence/result commit（`646dbda`）として自己参照しない。**旧記録の「12ファイル未commit dirty」は、`68fbbba`（回帰test commit）で12ファイルが全てcommit済みとなったため削除する。
...
- **static/direct evidence:** migration/static/JS **35 / 0 / 0 / 0 / BUILD SUCCESS**。20クラスdirect consumer regression **150 / 0 / 0 / 0 / BUILD SUCCESS**。CI相当`verify-like-ci.ps1`はDocker `29.6.1`/Node `v24.18.0`で**1471 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**（01:09 h）、skip確認 **0 test cases / 0 report classes**、script exit **0**。L4 zero-skipped確認済み（再取得不要）。今回`FlywayV79_1RepairSmokeTest`へpartial-state testを1メソッド追加したため次回全量は1472件になるが、当該クラスは実MySQLで**2 / 0 / 0 / 0**を確認済み。
- **V79.1専用実MySQL evidence:** `FlywayV79_1RepairSmokeTest`をDocker Server `29.6.1`で実行し、**2 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**（test execution 1998 s / Maven total 34:17）。既存のlegacy checksum・repair単独危険性・forward repair・partial history・clean/re-forwardに加え、今回`v79_1-fk-actions-forward-repair.sql`を`information_schema`状態判定で再開可能に改修し、**DROP後・FK追加後・CHECK追加後の各partial状態から再実行**して最終schemaとV79.1限定repair/validateへ収束することを実MySQLで検証した（各状態でrunbook2回実行の冪等性も確認）。
- **scheduler H2 evidence:** production既定の`app.scheduler.lock.use-db-time=true`を維持し、`test` profileだけ`false`へ切り替え。`SchedulerLockH2IntegrationTest`で**3 / 0 / 0 / 0**。
- **複数JVM / rollback / Webhook evidence:** `OperationalBoundaryMySqlIntegrationTest` **3 / 0 / 0 / 0**（2 JVM・1共有DBのShedLock/claim、`LOCK_NOT_ACQUIRED`/`CLAIM_RESULT=0`、commit前例外時実DB rollback）。`WebhookNotifierLoopbackIntegrationTest` **1 / 0 / 0 / 0**（loopback実HTTP JSON POST）。
- **desktop/390px 5業務browser Demo evidence:** 実Chrome `150.0.7871.187`（Playwright、headless実ブラウザ）で**10経路（見積提出・契約稼動化・請求送付・BP支払確定・月次締め × desktop/390px）全てPASS**。各経路で（a）申請者単独では対象状態が変わらない（申請者単独確定不可）、（b）申請→承認→適用で対象状態が1回だけ変わる（適用1回）、（c）申請時の二重click/retryでも申請1件のみ（idempotency key一意制約）、（d）承認時の二重clickでもAPPROVE action 1件・retry後も業務操作は再適用されない。証拠は`evidence/browser-m/`配下のスクリーンショット40枚＋JSON 11ファイル（各経路のbefore/after状態・申請数・action数・retry安定性）に記録済み。
- **残存gateのN/A化（「必要な場合」のまま放置しない）:** （1）**2つのfull application instance cron**はrequirements R2.3/R2.4/R4.2が要求する性質（commit後outbox配信・再送で二重外部連携を作らない）をDBレベルのShedLock/outbox claimで実現しており、デプロイ構成として2 instanceを要求していない。このDB境界の性質は`OperationalBoundaryMySqlIntegrationTest`で実測済みのため、正式要件外として**N/A**。（2）**外部provider相当のWebhook endpoint到達**はR2.3が要求する「transaction内で外部APIを呼ばない」こと（呼出タイミング）を指しており、特定の実provider統合は要求していない。Webhook URLは`m_system_config`の設定値（未設定時は`SENT`扱い、design §1.2）で、実HTTP送信経路はloopbackで実測済みのため、正式要件外として**N/A**。
...
- **rollback:** 文書・コード変更はHead `68fbbba`に対してcommitする。V75〜V79は編集・削除しない。V79.1適用済みDBを戻す場合はbackup復元または新しいforward migrationを使用する。
...
| `R4-REVIEW-04` | **OPEN（文書整合の独立再Review待ち）** | Packet・manifest・中央ledgerをcode baseline Head `68fbbba`（Base→Head=**23 commits/219 files/+11639/-337**、当時値）と初回Review evidence commit `2978461`（履歴）・現行Review evidence/result commit `646dbda`（27 commits/274 paths）へ同期したが、独立Reviewで(row7前半の旧記載・manifest §5.1のBP固定ID表記・README seed説明・Packet commit記述)の文書不整合が指摘され、修正後の再確認前 | 修正後の文書整合を独立Reviewで再確認し、問題なければクローズ |
...
**現行判定:** **REVIEW（独立再Review待ち）**。R4-REVIEW-01/02/03とR4-P1-01はVERIFIED_CLOSED、B1/T046・M/T047は`[x]`化（技術・機能gate確認済み）、R4-REVIEW-04は文書整合の独立再Review待ち。S07はREVIEW状態で、文書整合の独立再Reviewを待つ。再Review確認後、S07 PASS・S09 READY・Wave 2解放へ一括遷移する。独立Reviewは本Packet（219-path inventory、`evidence/browser-m/`、実MySQL証拠、Head分離）を対象に最終確認する。
