...
- **Review対象production Head:** Review Base=`5d228d211d0d752833fe3424a3b8aa4b40096733`、Head=`6680e7d81c7842262a2fd07c57fb9942e80573ce`。`HEAD = origin/main = origin/HEAD`、branch=`main`を確認した。`HEAD^`は`1e8a2243e0ed095bb211519b1242d4bcff17caef`であり、旧Packet・修正履歴としてのみ扱う。
- **scope:** Base→Review対象production Headは**22 commits / 212 files / +10459/-330**。212 pathsはすべてproduction Headにcommit済みの範囲である。
- **Packet文書commit / provenance:** **独立Packet commitなし。Packet文書4ファイルはReview対象production Head `6680e7d81c7842262a2fd07c57fb9942e80573ce`に同梱済み**であり、Head自身をPacket文書commitとして自己参照しない。確認開始時のproduction worktreeはcleanで、今回の文書同期、V79.1専用回帰追加、scheduler H2設定・回帰追加後は12ファイル（4資料、V79.1専用test/runbook、scheduler設定Java・main/test YAML、新規H2 lock回帰test、複数JVM/rollback境界test、Webhook loopback test）の未commit編集によりdirtyである。これらはReview対象production Head外の作業木差分として管理する。commit/pushは行わない。
- **R4-P1-01:** V79.1、`applicant_role_condition`、`t_approval_responsibility`、PERMISSION_GROUP/ORGANIZATION_MANAGER/FINANCE_MANAGER、as-of/scope/fail-closedを実装済み。V75〜V79は変更していない。V79.1のFK action変更（`CASCADE/SET NULL`→`RESTRICT`）は現行Review対象production Head `6680e7d81c7842262a2fd07c57fb9942e80573ce`に同梱済みであり、Headの212-path統計・manifestに含まれる。APPLICANT_MANAGER（`t_user_organization.manager_user_id`）のvalid_from/valid_to両端境界、所属期間外、manager `NULL`/無効/削除済み、候補0件、申請者本人しかmanager候補にいない場合、mapper境界の不存在IDとsnapshot固定の回帰を`RouteResolverServiceTest`へ追加済み。`manager_user_id`はMySQL V60と共有H2 schemaの双方でFK制約があるため、存在しないIDの直接fixtureはできない。代わりにH2で`SysUserMapper.selectById`の不存在IDが`null`になる境界と、DBで表現可能な`NULL` assignmentのfail-closedを検証し、MySQLでは`fk_user_org_manager`の存在を検証する。`ApprovalAdministrationApiControllerTest`にresponsibility逆期間がHTTP 400かつApiResult.code=400になるテストを追加済み。
...
- **static/direct evidence（作業木追加回帰後）:** migration/static/JSは**35 / 0 / 0 / 0 / BUILD SUCCESS**。R4-P1-01（7クラス）＋B1/M（13クラス）の**20クラスdirect consumer regressionは150 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**。CI相当`verify-like-ci.ps1`の最終実測はDocker `29.6.1`、Node `v24.18.0`、`mvn -B clean test` **1471 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**（01:09 h）、skip確認 **0 test cases / 0 report classes**、script exit **0**。L4 zero-skippedはこのrunで確認済みであり、Maven本体の結果とscriptのskip gateを分離して記録する。
- **V79.1専用実MySQL evidence（現作業木）:** `FlywayV79_1RepairSmokeTest`をDocker Server `29.6.1`で実行し、**1 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**を確認した。V79 legacy→V79.1 upgrade、`flyway_schema_history`、旧checksumによるvalidate失敗、`repair()`単独ではFK/CHECKを変更しないこと、forward DDL→schema assert→V79.1限定allowlist repair→validate、最初のALTERだけが残ったpartial history→repair→再migrate、`clean()`後のV79→V79.1再適用、FK action/CHECK/index/違反拒否を実MySQLで確認済みである。`repair()`単独を安全な修復手順とは扱わない。所要時間はtest execution `757.1 s`、Maven total `13:56`。
- **scheduler H2 evidence（現作業木）:** production既定の`app.scheduler.lock.use-db-time=true`（環境変数`SCHEDULER_LOCK_USE_DB_TIME`で上書き可）を維持し、`test` profileだけ`false`へ切り替えた。`SchedulerLockH2IntegrationTest`でH2のlock取得・同一lock競合・解放後再取得を**1 / failures 0 / errors 0 / skipped 0**、既存`TaskNotificationSchedulerH2Test`と`NotificationOutboxSchedulerIntegrationTest`を含む3クラス合計を**3 / 0 / 0 / 0 / BUILD SUCCESS**で確認した。`usingDbTime()`由来の`DB time is not supported for unknown database product`例外は targeted logに現れず、lock取得失敗を`assertDoesNotThrow`/取得assertで検知する回帰を追加した。\n- **operational boundary実MySQL evidence（現作業木）:** `OperationalBoundaryMySqlIntegrationTest`をDocker Server `29.6.1`で実行し、**3 / failures 0 / errors 0 / skipped 0 / BUILD SUCCESS**を確認した。独立JVMのShedLockは`LOCK_ACQUIRED pid=...`→`LOCK_NOT_ACQUIRED pid=...`→`LOCK_RELEASED`→`LOCK_ACQUIRED pid=...`、outbox claimは`CLAIM_RESULT=1`/`CLAIM_RESULT=0`、DB状態`PROCESSING`、`attempt_count=1`を確認した。adapter後の意図的例外ではapproval action/request/Quotation対象行が実MySQL transactionでrollbackした。
- **Webhook loopback evidence（外部providerとは分離）:** `WebhookNotifierLoopbackIntegrationTest`は**1 / failures 0 / errors 0 / skipped 0**。`127.0.0.1`のloopback endpointへ実`RestTemplate`でJSON POSTが到達することを確認したが、外部Webhook providerは呼び出しておらず、そのgateはOPENのままである。
- **Chrome login DOM smoke（browser受入Demoとは分離）:** Chrome ProductVersion `150.0.7871.187`でlocalhost `/login`をdesktop `1440x900`とmobile `390x844`から取得し、双方HTTP `200`、DOM bytes `5678`、login/form検出を確認した。これはbrowser pathのfeasibility smokeに限られ、見積・契約・請求・BP支払・月次締めの5業務申請→承認→適用DemoをPASSへ読み替えない。
- **未達gate:** 2つのfull application instanceによるcron end-to-end（必要な場合）、外部provider相当Webhook到達、desktop/390pxでの5業務browser Demo、今回の作業木修正後の独立Reviewは未完了であり、個別に実測できるまでPASS扱いにしない。shared JDBCの複数JVM ShedLock/claim、commit前例外時rollback、loopback送信、login DOM smokeは実測済みだが、これらを未達gateの代替として過大評価しない。定向greenやMaven BUILD SUCCESSをrelease PASSへ読み替えない。
...
| `R4-REVIEW-03` | **OPEN** | B1/T046・M/T047 checkboxは`[ ]`。shared JDBCの複数JVM ShedLock/claim、commit前例外時rollback、loopback webhookは実MySQL/loopbackで測定済みだが、full application instance cron（必要な場合）、外部provider、5業務browser Demoが未達。CI相当L4は1471/0/0/0、zero-skippedを確認済み | full application instance、外部provider、5業務browser、独立Reviewを実証 |
| `R4-REVIEW-04` | **OPEN** | Review対象production Head `6680e7d`、22 commits、212 filesを確認し、Packet文書4ファイルが同Headに同梱され独立Packet commitがないことを確認したが、今回の文書編集後の独立再Reviewは未了 | current HeadとPacket provenanceの分離、manifest完全性、通常Review再開根拠を独立確認 |
| `R4-P1-01` | **OPEN / P1** | source別境界/異常系回帰、V79.1の実MySQL fresh/legacy適用・履歴・checksum/FK/CHECK/index assertion、V79.1-specific partial/repair/rollback、shared JDBCの複数JVM ShedLock/claim、commit前例外時rollback、loopback webhookはgreen。外部provider、full application instance cron（必要な場合）、browser等の残存実環境gateと独立再Reviewが未完了 | route/source/snapshotの実環境gate、残存release gate、独立再Reviewを完了するまでOPENを維持 |
