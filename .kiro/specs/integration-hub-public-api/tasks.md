...
- Initial delivery binding: `IntegrationHubWebhookDeliveryBindingValidator`がenqueue保存前にclient bindingからprimary opaque IDを再計算し、
  envelopeとprimary DTO fieldを検証する。workerもclaim後・外部HTTP前に同じvalidatorを実行し、不一致rowを`PRIMARY_BINDING_INVALID`で送信せずFAILEDへ収束する。
  DuplicateKey収束はpayload hash・primary type・primary IDを同時比較し、同一payload・別primary、type/ID不一致を拒否する。
...
- Test requirements: 初回enqueueのprimary type/IDとopaque IDの不一致、DuplicateKey競合での同一payload・別primary、worker送信前のbinding不一致でtransport未実行を追加する。
- Remediation evidence: `30199db8`、`2684ff8f`、V133、V134、`IntegrationHubWebhookResourceScopeMapper`、`IntegrationHubWebhookDeliveryBindingValidator`、H2 retention/schema/replay/mapper、MySQL 8 concurrency/retentionを追加。focused/H2 44/44、MySQL 8/8、ApiDelivery 7/7、worker 10/10はfailure/error/skipなしでPASS。
