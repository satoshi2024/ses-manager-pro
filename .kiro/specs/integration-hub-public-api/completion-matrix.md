# NF-05 完了対応表（Owner承認済み・R-NF05 Plan PASS・F1実装完了）
...
| F1 DDL | approval-decision, tasks, design, V129, H2 schema, entity/mapper/service/crypto, F1 tests | client/credential/scope/idempotency/usage bucket/nonce/webhook/retention persistence基盤。対象F1 suite 23 tests PASS、MySQL Flyway smoke PASS | COMPLETE（Implementation Review待ち） | implementation `a7654b44` → docs証跡commit後にremote固定 |
...
- F1 targeted suite: 23 tests、failure 0、error 0、skip 0。
- 全fast suite: F1失敗なし。ただし既存loopback接続・production-config系10 errors、既存規約/cache headerの
  2 failuresが残るため、全体PASSとは扱わない。
- 状態: F1実装完了、独立Implementation Review未実施。F2以降とproduction enablementは未着手。
