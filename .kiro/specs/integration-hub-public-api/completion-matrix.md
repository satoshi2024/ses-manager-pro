# NF-05 完了対応表（scope expansion承認済み・F2 remediation・独立Review継続中）
...
| F2 security chain | tasks/design/inventory、`src/main/java/com/ses/config/integrationhub/`、F2 tests、V130 | `@Order(0)`専用chain、stateless、HMAC byte canonical、connector raw-target、trusted proxy/CIDR、nonce、typed effective scope、route/quota、専用audit、有限metrics、deny-only、stable error | IMPLEMENTATION_REMEDIATION_REVIEW_PENDING（f57df6d2のP1/P2を追加remediate） | 初回 `aadcfa98`、`e47025b5`、追加 `a16cdcba`。独立再Review待ち、最終remote Headは外部handoffで固定 |
| F2 Implementation Review remediation | F2専用chain、V130、H2 schema、F2 tests | raw request-target供給、client×route scope intersection、audit一request一record、strict IP、metrics cardinality、namespace root | REMEDIATED_REVIEW_PENDING | fixed FAIL Head `220ac86f` → `e47025b5`（6件）、fixed FAIL Head `f57df6d2` → `a16cdcba`（2件）。19追加tests PASS |
...
Plan deltaはca27f455でPASS済み、F2はFAIL後のremediation済みで独立再Review待ち。A2はN/Aで全体完了をblockしない。
...
F2 implementation evidence: 初回 `aadcfa98`、remediation `e47025b5`、追加remediation `a16cdcba`（connector raw-target、authoritative tenant/legal scope、audit V130、strict/mapped IP、metrics、namespace root、production enablementなし）。最終remote Headは外部handoffで固定する。
