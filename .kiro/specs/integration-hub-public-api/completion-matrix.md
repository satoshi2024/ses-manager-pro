# NF-05 完了対応表（R-NF05 REV3 IMPLEMENTATION PASS・PR #97 作成済）
...
| B2 inbound/DLQ/admin UI | tasks/design/requirements/inventory、V135/V136/V137、inbound/controller/admin/replay classes、B2 tests | client/provider/event unique、strict parser、raw hash/allow-list snapshot、duplicate/conflict、claim/lease/CAS、DLQ、admin replay、current scope再検証、safe UI、独立metadata retention、stable external error boundary、`receiptForSameHash` | IMPLEMENTATION_PASS | fixed Head `eac98db0`、R-NF05 REV3 P0/P1/P2=0/0/5 |
...
| M verification | tasks/design/evidence-index、M tests、runbook | penetration、failure drill、rate boundary、rotation/revoke、secret scan、runbook | IMPLEMENTATION_PASS | fixed Head `eac98db0`、R-NF05 REV3 |
...
| NF05-IMPL-B2-006〜007（P1×1/P2×1） | `7f16cc1d9aecf3ebd688d69f981f0610567d4d1` | `251461f1` | SPEC_ADDRESSED（独立再Review待ち） |

| NF05-IMPL-B2-008（P1×1） | `7757bfa49a4ece9aceddcedde2e835bc7466afe1` | `e564f400` | SPEC_ADDRESSED（独立再Review待ち） |
...
初回送信前primary bindingの独立Review findingを、`c2cbfb99133d0df3f8d5eee285be340163747e31`でremediateした。client bindingからprimary type/内部IDのopaque IDを再計算し、envelopeの`publicResourceId`とprimary DTO fieldの双方を一致検証する共通validatorをenqueue保存前とworker外部HTTP前へ配置した。DuplicateKey収束でもpayload hash、primary type、primary IDを同時比較し、不一致を拒否する。

| Finding | 対応 | Status |
|---|---|---|
| NF05-IMPL-B1-008 | 初回enqueue、worker送信前、DuplicateKey再読込のidentity bindingを共通化し、primary ID/type不一致、同payload・別primary、transport未実行をtest | SPEC_ADDRESSED（独立再Review待ち） |
...
Plan deltaはca27f455でPASS済み、F2はfixed Head `d022e60039880dc5d4743f336661819cda7fc3f4`でIMPLEMENTATION PASS、A1はfixed Head `69f857d3ac7d513b66265b02871688b28d2e7e5d`で独立Implementation Review PASS。B1は初回Review FAIL（fixed Head `0f1a92974ea914d16de07ccf5a586fac215283f0`、P0=0/P1=4/P2=1）を`30199db8`、再Review P1-006/P1-007を`2684ff8f`、追加remediationを`5c94367c` → `0618d983` → `c2cbfb99`で実施済み・独立再Review待ち。A2はN/Aで全体完了をblockしない。
...
B1 implementation/remediation evidence: `971c17d7`でNF-05専用`t_api_delivery`を再利用するoutbound worker、HMAC-SHA256 signer、MOCK/STUB/LOOPBACK transport、
DLQ replay audit、V132を追加し、`30199db8`で署名/envelope binding、replay current authorization、V133 audit/payload retention分離、fresh clock/CAS、
failure/concurrency証跡を追加した。`2684ff8f`ではauthenticated internal admin principal/action permission、operatorRef入力排除、numeric scopeからの
opaque ID再計算照合、resource membership fail-closedを追加した。focused unit/H2/MySQL suiteはfailure/error/skipなしでPASSした。外部I/OはDB transaction外であり、
...
| B2 implementation commit | `122c7c3bb5653eb788d58040c6defc816ff67013` → `e564f400db7753226ae5d54ade802197e67be1a3` |
