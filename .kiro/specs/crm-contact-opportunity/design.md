...
- **逸脱と根拠（legacy repair）**: V73/V74は適用済みのため編集しない。RepeatableはV73より前の
  ...
  indexを追加する。leadのcreate/updateは同じ正規化規則でキーを維持する。同期表はV73/V74/V1
  （不変）、`V74_1`、`V74_2`、`schema-crm-h2.sql`、`engineer-schema-h2.sql`、MySQL smoke assert、
  entityの7点とし、R__の再実行は欠落列・indexだけを変更する。
