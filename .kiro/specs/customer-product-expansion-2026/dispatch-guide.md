...
4. BP masterとCRMは別AIへ並行分派可。migration基盤taskはBP V70/V71（merge済み）→CRM V73の順にmergeする。
5. approvalはBP/CRM merge後。予約はV72だが、CRM V73が先にmergeされている場合は
   V74へ繰り上げる（Flywayは`out-of-order`無効。前の欠番は埋めない）。
