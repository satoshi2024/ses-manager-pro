...
- V59は作成せず永久欠番として保持する。V60〜V75は実装前の計画値に過ぎず、現時点の最大番号はV58である。将来共有DBを再開する場合もV59を補完・再利用せず、その時点のFlyway最新番号`latest + 1`から再計画する。
...
1. **採番の現状**: 現行migrationはV1〜V58のうち19、23、41、47を欠番として持ち、V59以降は未作成である。V59は永久欠番とし、V60〜V75の計画値も実装前に当時のFlyway最新番号`latest + 1`から振り直す。V59の補完・再利用、過去migrationの編集・out-of-order適用は禁止する。
2. **実行順の文書表記**: 旧版READMEの一覧表示にあったapproval(V66)とCRM(V65)の順序問題は、今回CRM(V65) → approval(V66)へ修正した。`parallel-execution-plan.md:56-58` と `dependency-matrix.md:7-12` を含め、BP(V64) → CRM(V65) → approval(V66)で一致している。番号自体は実装前の計画値であり、実際の採番はV59永久欠番を除外した当時のlatest+1規則に従う。
