...
| 2-B | dispatch（T060〜T066）とattendance（T067〜T074） | order独立Review合格、G2/G6確定 | V82→V83 | Contract担当メソッドと雇用勤怠テーブルを分離 |
| 2-C | staffing（T075〜T080）単独 | dispatch/attendance完了 | V84 | proposal/contract/availabilityを統合参照 |
| 3-A | external portal（T081〜T087）とengineer portal（T088〜T093）は条件付き | Wave 2完了、G3/G8/G9方針確定 | V85→V86 | `SecurityConfig.java`はexternal portal統合担当のみが先に変更・merge |
| 3-B | accounting（T094〜T101）単独 | portal系、order、BP、archive完了、G4確定 | V87 | Freee adapter、invoice、BP paymentを変更 |
| 3-C | JP PINT（T102〜T108）単独 | accounting完了、G5確定 | V88 | CanonicalInvoiceと会計/請求境界を固定後に開始 |
| 4 | AI feedback（T109〜T115）単独 | CRM、proposal、staffing、outcome source完了 | V89 | 学習指標の母集団とPII境界を先に固定 |

> **採番の正本は `README.md` の予約表**であり、S07の正式migrationはV75〜V79、S09はV80＋V81（R10順方向修復）、S10〜S17はV82〜V89である。S09実装Headで確認した実適用済みの最新は **V81** である。
