...
| 2-C | staffing（T075〜T080）単独 | dispatch/attendance完了 | V99 | proposal/contract/availabilityを統合参照 |
| 3-A | external portal（T081〜T087）とengineer portal（T088〜T093）は条件付き | Wave 2完了、G3/G8/G9方針確定 | V100→V101 | `SecurityConfig.java`はexternal portal統合担当のみが先に変更・merge |
| 3-B | accounting（T094〜T101）単独 | portal系、order、BP、archive完了、G4確定 | V102 | Freee adapter、invoice、BP paymentを変更 |
| 3-C | JP PINT（T102〜T108）単独 | accounting完了、G5確定 | V103 | CanonicalInvoiceと会計/請求境界を固定後に開始 |
| 4 | AI feedback（T109〜T115）単独 | CRM、proposal、staffing、outcome source完了 | V104 | 学習指標の母集団とPII境界を先に固定 |

> **採番の正本は `README.md` の予約表**であり、S07の正式migrationはV75〜V79、S09はV80＋V81（R10順方向修復）、S10=V84、S11=V83、S12〜S17=V99〜V104である。CI/TestcontainersでV83の実適用を確認したため、V82は欠番として保持する。S11方式A追補（R2-P1-02）のV91は2026-08-09に発注者割当で実在し、S12〜S17の予約はV99〜V104へ繰り上げ済みである。永続環境は個別read-only証跡をpacketへ記録し、着手時は必ず`db/migration`の実ファイルを再確認する。
