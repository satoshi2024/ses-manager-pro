...
| 1-A | BP master（T034〜T040）とCRM（T048〜T053） | Wave 0完了 | BP V64→CRM V65 | 実装は並行可だがDDL merge/deployは番号順。approvalは両方待つ |
| 1-B | approval（T041〜T047）単独 | BP/CRM完了 | V66 | Contract/Invoice/BP payment共通経路を変更 |
| 2-A | order（T054〜T059）単独 | approval完了 | V67 | 契約・請求状態機械の基礎 |
| 2-B | dispatch（T060〜T066）とattendance（T067〜T074） | order完了、G2/G6確定 | V68→V69 | Contract担当メソッドと雇用勤怠テーブルを分離 |
| 2-C | staffing（T075〜T080）単独 | dispatch/attendance完了 | V70 | proposal/contract/availabilityを統合参照 |
| 3-A | external portal（T081〜T087）とengineer portal（T088〜T093）は条件付き | Wave 2完了、G3/G8/G9方針確定 | V71→V72 | `SecurityConfig.java`はexternal portal統合担当のみが先に変更・merge |
| 3-B | accounting（T094〜T101）単独 | portal系、order、BP、archive完了、G4確定 | V73 | Freee adapter、invoice、BP paymentを変更 |
| 3-C | JP PINT（T102〜T108）単独 | accounting完了、G5確定 | V74 | CanonicalInvoiceと会計/請求境界を固定後に開始 |
| 4 | AI feedback（T109〜T115）単独 | CRM、proposal、staffing、outcome source完了 | V75 | 学習指標の母集団とPII境界を先に固定 |
