...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T072完了・T071独立Review **PASS**、T072独立Review待ち、T073〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED、NOTE-R4-02 FIXED、NOTE-R4-01記録済み、leave 24/0/0/0、22 class全green、skip 0**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1 freee/provider sync、`840539da`）実装済み: AttendanceProvider interface＋mock/freee実装（OAuth/refresh共通基盤apiGet/apiPost、401 refresh 1回/429 backoff/timeout/冪等キー・相関ID/秘密非ログ）、冪等push・cursor差分pull・締め済み/承認済み月拒否→t_overtime_followup finding＋通知、CSV fallback、run/status/export-csv API、管理画面同期カード、DDLなし**。新規5 class 29/0/0/0、指定回帰165/0/0/0 skip 0。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` | R11 Round 4 fix delta独立検証: leave 4 class **24/0/0/0**、attendance/approval/overtime/integrity 22 class全green、skip 0。T072: 新規5 class **29/0/0/0 skip 0**、指定回帰 **165/0/0/0 skip 0**、`git diff --check` PASS。P0=0/P1=0/P2=2（R2-P2-01/02、T074/M再評価）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T072独立Review→T073（B2. 客先工数差異/通知）着手**。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
