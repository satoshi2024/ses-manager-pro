...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 / R4-T01〜R4-T08 | `IN PROGRESS (Stage A: SpecHead Revision 5)` | Base `f8b81e77` に対し Stage A SpecHead Revision 5 (OAuth敗者TOKEN_REFRESH_IN_PROGRESS/RETRYABLE(5s)遷移、Backup完全DDL・information_schemaガードRollback SQL・Flyway repair、freee開発者リファレンスURL・固定contract fixture・M CONDITIONAL PASS、経費t_engineer_accounting_history asOf照合・UNKNOWN fail-closed、Tenant別accounting.timezone.{tenantId}・AccountingTenantContextHolder) を確定・承認申請中。Stage B 実装は未着手・未解放 | S15 R4 是正 / Stage A SpecHead Revision 5 | Base `f8b81e77` | Direct 50/50, MySQL 5/5, Fast 2381/2381 PASS (コード実装済み、Spec 整合・深化のため Stage A 改訂中) | Stage A SpecHead 承認 → Stage B 実装・検証 |
