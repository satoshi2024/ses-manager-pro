...
| 8 | 1 | `crm-contact-opportunity` | T048〜T053 | **`IN PROGRESS`** | Round 6再ReviewのP1-01（source_cost型）/P1-09（lead候補全件走査）を`ac43f31`で修正。P2-02（primary同時作成）/P2-03（rollbackのproposal列）も対応済み。残りはL4全量とdesktop/390px全role browserのrelease hard gate | S08 crm-contact-opportunity Round 6対応 | Base `8e5066a` → Head `ac43f31`（`main` / `origin/main`） | V73/V74/V74.1を不変のままV74.2 forward-fixを追加。定向/H2 39/0/0、MySQL fresh/legacy/partial/repair/concurrency 各1/1・0 failure/error、MigrationScriptIntegrity 25/25、compile、Node 3/3、`git diff --check` PASS。Round 6 P1=0（2件 VERIFIED_CLOSED） | L4全量とdesktop/390px全role Demoを完了し、独立再ReviewでPASS確認するまでS07 approval/次Waveを解放しない |
