...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060はR10 Round 4 PASS、R4-P1-01 IMPLEMENTER FIX提出） | order PASS、G2確定済み。CI/TestcontainersでV83実在・V82不在をread-only確認し、正式decisionによりS10をV84へ繰り上げ。R4-P1-01のR10独立確認までdeploy freezeを維持し、T061/V84/production変更は開始不可 | S10 dispatch-outsourcing-compliance-ledger 実装・R10 fix delta | Base `4789c192a733d2d64b13c9941ab53e3780aefbe9` → current Head（main / origin/main） | T060 PASS。R4-P1-01 guard、CI/Testcontainers evidence、予約資料・fixture同期、SpecDispatchConsistencyTest direct regression PASSを提出対象とする | CI/local/evidence packet、formal migration decision、全派工資料同期、legacy fixture、SpecDispatchConsistencyTest PASSをR10が確認しR4-P1-01をVERIFIED_CLOSEDとするまでT061開始不可 |
