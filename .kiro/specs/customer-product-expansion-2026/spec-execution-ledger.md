...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T072完了・T071/T072独立Review **PASS**、T072 R5-P2-01/02独立再Review待ち、T073〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS（P0=0/P1=0/P2=4/NOTE=3）**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1 freee/provider sync）: AttendanceProvider interface＋mock/freee実装（apiGet/apiPost共通基盤、401 refresh 1回/429 backoff/timeout/冪等キー・相関ID/秘密非ログ）、冪等push・cursor差分pull・締め済み/承認済み月拒否→t_overtime_followup finding＋通知、read-only照合（R5-P2-02: 一致/差異/該当なし＋差異サンプル）、timezone tenant設定（R5-P2-01）、CSV fallback、run/status/export-csv API、DDLなし**。R5 fix delta `c750fcb3`（P2×2 FIXED_BY_IMPLEMENTER・独立再Review待ち）。新規31/0/0/0＋回帰136/0/0/0 skip 0。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` → T072完了 `82b8612c` → R5 fix `c750fcb3` | R11 Round 5独立検証: T072新規5 class **28/0/0/0**＋attendance回帰214件（唯一のFAILはcross-lane NOTE-R4-03）skip 0。R5 fix delta: 新規 **31/0/0/0**＋指定回帰 **136/0/0/0 skip 0**、`git diff --check` PASS。P0=0/P1=0/P2=4（R2-P2-01/02、R5-P2-01/02 FIXED_BY_IMPLEMENTER）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T073（B2. 客先工数差異/通知）着手可**（R11 Round 5）。R5-P2-01/02の独立VERIFIED_CLOSED後が安全。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
