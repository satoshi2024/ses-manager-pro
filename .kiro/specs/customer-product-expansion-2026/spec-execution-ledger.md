...
| 17 | 4 | `ai-feedback-learning` | T109〜T115 | **`IN PROGRESS`（T109完了、T110未着手）** | G10はmock/rule既定・実データ外部送信禁止を記録（`GATE-S17-G10-PROD`は本番gate）。allowlist/mask/metric正本は `ai-feedback-learning/g10-pii-allowlist.md`。F1（V108）は T110 から。着手時に Flyway latest を再確認し衝突なら後発を上へ採番 | S17 ai-feedback-learning 実装 | Base（S16 CONDITIONAL PASS Head `07ae187c`） | T109 L0。独立Review未開始 | T110（V108 DDL）はT109 checkbox後。R17 PASSでroadmap完了 |
