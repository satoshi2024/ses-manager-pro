...
| 8 | 1 | `crm-contact-opportunity` | T048〜T053 | **`IN PROGRESS`** | Round 4独立Review = **FAIL（P0=1/P1=4/P2=11/NOTE=7）**。P0-01は`4058d9b`で修正済み（R__余分`END IF`削除＋静的均衡検査）。P1/P2対応は本Roundで継続。migration実績は**V73（DDL）/V74（action permission seed）**、6列legacy-repair例外はdesignへ根拠固定 | S08 crm-contact-opportunity Round 4対応 | Base `8e5066a` → 対応commitはreview-ledgerへ固定予定 | 実測前。`MigrationScriptIntegrityTest`はP0修正済み。Docker fresh/legacy/partial/repair/L4、roles_json MySQL往復、browser DemoはOPEN（Docker/browser不在） | P1/P2の定向test greenとDocker L4証拠commit固定までS07 approvalを解放しない |
