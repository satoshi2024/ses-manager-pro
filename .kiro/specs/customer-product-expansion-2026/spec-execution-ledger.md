...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 / R4-T01〜R4-T08 / R5-R1 / R6-R1 | **`CONDITIONAL PASS`** | Stage A SpecHead Revision 6 PASS（`e0d8a96f`）。Independent Re-Review R7はBase `951ac238` → Head `a2ab297d`、P0=0/P1=0/P2=0、総合CONDITIONAL PASS。R3-P0-01/R6-R1はVERIFIED_CLOSED。S15本番deployは`GATE-S15-FREEE-PROD`と実2 JVM 401競合確認まで保留する | S15 R7 Review結果・ledger収束 | R5 review Base `7219fd2a` → prior FixHead `fe69dd3a` → R5-R1 fix `d6b8c307` → R5 Packet `951ac238` → R6 implementation `186b746c` → R6 Packet/R7 Review sync `a2ab297d` | Static 38/38、実MySQL V106.2旧公開history回帰 2/2。既存Gate証跡: Fast 2435/0/0/0、MySQL 57/0/0/0、Performance 1/0/0/0、Browser 1/0/0/0、Backup SUCCESS（RPO 60秒、RTO 14400秒、secret scan 0）。全skip 0、`git diff --check` PASS | `GATE-S15-FREEE-PROD`と実2 JVM 401競合はRelease Gate。S15本番deploy不可。S16は独立Review CONDITIONAL PASS済（Head `07ae187c`） |
| 16 | 3 | `jp-pint-digital-invoice` | T102〜T108 | **`CONDITIONAL PASS` / Head `07ae187c`** | 独立Review Round 5: P0=0 / P1=0。R5-P0-01・P1-06・R5-P1-01・P1-01 は `07ae187c` で VERIFIED_CLOSED。**release gate = `PENDING_SANDBOX`**（第一会計 Peppol sandbox 未取得のため B1/B2/M および本番は PASS にしない）。A1 browser Demo・仕入ジョブ接続等は NOTE / 本番 gate で S17 開始を阻害しない | S16 jp-pint-digital-invoice 実装（Round 5 まで） | Base（S15解放後）→ Head `07ae187c6cb040f46cfed0fdebba6c18d94653c4`（`main` / `origin/main`） | **CONDITIONAL PASS**（P0=0/P1=0 / `PENDING_SANDBOX`）。mock 定向は通過 | **S17（`ai-feedback-learning`）解放可**。次は T109（G10 spike）。sandbox は S17 の开工 blocker ではない |
| 17 | 4 | `ai-feedback-learning` | T109〜T115 | **`CONDITIONAL PASS` / Head `6b4cc56f`** | Round 3: P0=0 / P1=0。R2-P1-01〜04 と R1-P2-01〜03 は VERIFIED_CLOSED。**P2=5 OPEN**。release gate = `GATE-S17-G10-PROD`（DPA/署名。本番前）。最終/本番 PASS は出さない | S17 Round 3 ledger 同期 | Base `9d2c229b` → Head `6b4cc56f`（`main` / `origin/main`） | Review 定向 40/0/0/0。implementer L4 2506/0/0/0（T115 Head） | 次specは開始しない。P2 と G10 を閉じない限り最終 PASS にしない |
...
## 2.7 S16 CONDITIONAL PASS → S17 READY（T109）（2026-08-20）

独立Review（Round 5 / Head `07ae187c`）で S16 `jp-pint-digital-invoice` は **CONDITIONAL PASS**（P0=0 / P1=0 / release gate = `PENDING_SANDBOX`）。
OPEN の P0/P1 は無い。Peppol sandbox は S16 の B1/B2/M および本番のみを留め、S17 の开工条件ではない（S15 CONDITIONAL PASS 後に S16/T102 を開いたのと同型）。

- 上表の番号16行を `READY（T102 spikeのみ開始可）` → **`CONDITIONAL PASS` / Head `07ae187c`** へ転記。
- 番号17行を `NOT READY` → **`READY`（T109 可開始）** へ転記。
- S16 production code の追加変更は行わない。S17 は T109（G10 spike・docs）から着手する。
