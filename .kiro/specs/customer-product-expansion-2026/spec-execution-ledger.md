...
| 7 | 1 | `approval-workflow-internal-control` | T041〜T047 | **`IN PROGRESS`** | T041(0)・T042(F1)完了。V75で5テーブル新設(route/route_step/request/action/delegation)、engine core実装。定向test 6クラス71件PASS。次はT043(F2: 5対象adapter)。migrationは**V75で確定**（CRMがV73＝DDL、V74＝権限seedを使用済み。V72は永久欠番、`out-of-order`は使わない） | S07 approval-workflow-internal-control 実装 | 未commit（working tree） |  | T043以降の実装継続。独立ReviewはF2合流後またはM前に実施 |
