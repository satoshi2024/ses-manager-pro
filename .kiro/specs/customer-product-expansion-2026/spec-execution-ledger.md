...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060はR10 Round 4 PASS、R4-P1-01 OPEN） | order PASS、G2確定済み。V83実在/V82予約の順序矛盾を環境別read-only証跡と正式decisionで解消するまでdeploy freeze。T061/V82/production変更は開始不可 | S10 dispatch-outsourcing-compliance-ledger 実装・R10 fix delta | Base `4789c192a733d2d64b13c9941ab53e3780aefbe9` → Head `79f63db0bc8fae23698b90517c4c35a621eb59b7`（main / origin/main） | T060 PASS。R4-P1-01 guard VERIFIED、issue OPEN / ENVIRONMENT_EVIDENCE_REQUIRED。Direct regression 8/1/0/0でS10 V82 <= latest V83を検出 | 全環境のV82/V83 flyway_schema_history証跡、V82→V83または採番繰上げの正式decision、予約表・全派工資料・legacy fixture同期、SpecDispatchConsistencyTest PASSをR10が確認後にT061開始可 |
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067/T068/T069/T070完了、T071〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。V83は実装済みだがV82 merge/apply後に適用 | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067成果 `93c1ac6` → T068実装 `b327b1b` → T069 calculator実装 `d395797` → T070 workflow `cc7c15c` | R11 P2-01はcurrent merged Head同期済みで独立再Review待ち。T068 H2 5/0/0/0、MySQL smoke 1/0/0/0、T069指定回帰70/0/0/0、T070指定回帰17/0/0/0。HR/法人別規程はATT-GATE-01〜06としてrelease gate | T071開始可。T070 code/台帳を独立Reviewへ提出し、V83のmerge/applyはV82後 |
