...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 / R4-T01〜R4-T08 | `IN PROGRESS (Stage A: SpecHead Revision 4)` | Base `f8b81e77` に対し Stage A SpecHead Revision 4 (SALES_INVOICE_CANCEL全状態拒否・種別×状態マトリクス完全定義、REAUTH_REQUIRED統一・50ページpagination・3回待機、V106.1完全Rollback SQL・Flyway repair、freee API v1公式一次資料・fixture契約・M CONDITIONAL PASS、実在エンティティ組織導出・asOf固定・全Consumer、AccountingTimezoneResolver、Demo付きTask台帳) を確定・承認申請中。Stage B 実装は未着手・未解放 | S15 R4 是正 / Stage A SpecHead Revision 4 | Base `f8b81e77` | Direct 50/50, MySQL 5/5, Fast 2381/2381 PASS (コード実装済み、Spec 整合・深化のため Stage A 改訂中) | Stage A SpecHead 承認 → Stage B 実装・検証 |
