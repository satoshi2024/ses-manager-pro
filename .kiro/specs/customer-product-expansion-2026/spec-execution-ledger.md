...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T074完了・T071/T072/T073独立Review **PASS・完結**、**T074（M）完了・独立Review待ち（S11最終）**） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**。**R11 R5-P1-01/R5-P2-03 fix delta再Review: VERIFIED_CLOSED**。**R11 Round 6 T073独立Review: PASS**。**R11 R6-P2-01 fix delta再Review: VERIFIED_CLOSED**。**T074（M）: L4全量1701 testsでattendance起因FAILゼロ**（5 failuresは全てattendance非関与: cross-lane 2・dispatch entity/H2不一致（NOTE-R6-03新規）1・scripts環境 2。38 skipped=Docker未起動のMySQL smoke）。browser Demo（desktop/390px実Chrome CDP）実測・`evidence/browser-m/`保存。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。cross-lane NOTE×4: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）、NOTE-R6-03（dispatch entity/H2不一致）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` ほか → T072完了 `9be5e5c` → T073 `5c34db26` → T073完了 `11398d9` → R6 fix `62e3d31` → R6転記 `ef4ce72` → T074完了記録 | R11 R6-P2-01 fix delta再Review: 独立実行 **234 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**。T074（M）: L4全量 **1701 tests / 5 failures（attendance起因0）/ 38 skipped（Docker）**、browser Demo 1/0/0/0、`git diff --check` PASS。P0=0/P1=0/P2=2（R2-P2-01 390px実測済み・R2-P2-02 paging）/NOTE=4（cross-lane＋R6-NOTE-02）＋NOTE-R6-03（dispatch）。HR/法人別規程はATT-GATE-01〜06として本番release gate | **T074独立Review（S11最終PASS）→S12（staffing-capacity-planning）解放**。統合担当はNOTE-R3-06/R4-03/R4-04/R6-03解消後、CI相当L4×1回（fresh/legacy MySQL smoke含む）。V83/V91/V98を変更・再適用せず、V82を補填しない |
