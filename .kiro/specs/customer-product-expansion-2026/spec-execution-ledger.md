...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T072完了・T071/T072独立Review **PASS**、**R5-P1-01 VERIFIED_CLOSED**、R5-P2-03 OPEN（T073着手時fix推奨）、T073〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**、R5-P2-01/02 FIXED。**R11 R5-P1-01 fix delta再Review: VERIFIED_CLOSED、新規R5-P2-03（cursor×scope外、P2）OPEN**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1 freee/provider sync）: AttendanceProvider interface＋mock/freee実装（apiGet/apiPost共通基盤、401 refresh 1回/429 backoff/timeout/冪等キー・相関ID/秘密非ログ）、冪等push・cursor差分pull（tenant timezone正規化）・締め済み/承認済み月拒否→t_overtime_followup finding＋通知・read-only照合（一致/差異/該当なし＋差異サンプル）・caller scope（管理者全件/HR法人/manager組織、scope外skip）・CSV fallback、DDLなし**。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` → T072完了 `82b8612c` → R5 fix `c750fcb3` → R5転記 `ba5625f3` → R5-P1-01 fix `1582d96` → R5-P1-01転記 `a955aa3` | R11 R5-P1-01 fix delta再Review: 独立実行 **218 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**、`AttendanceSyncServiceTest` **12/0/0/0**。P0=0/P1=0/P2=3（R2-P2-01/02、R5-P2-03）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T073（B2. 客先工数差異/通知）着手可**。R5-P2-03はT073着手時にpullのグローバル/スコープ運用方針と合わせてfixしdesign/tasksへ明記。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
