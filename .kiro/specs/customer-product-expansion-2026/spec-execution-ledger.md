...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS / FAIL`（T060〜T065 PASS。R22-P1-01/P1-03は`OPEN / MYSQL_VERIFICATION_PENDING`、P1-02/P1-04/P1-05は`FIXED_BY_IMPLEMENTER / MYSQL_VERIFICATION_PENDING`、P2-01は`VERIFIED_CLOSED`。同一HeadのCIは`1842/1/29/0`でBUILD FAILURE、G2実在証跡・Phase A/B・GATE-T066-HISTORY未達、T066 checkbox未完了） | order PASS、V84/V85実在。R19-P1-02/R19-P2-03/R20-P1-01 VERIFIED_CLOSED。R10がR21 canonical payload deltaを`ACCEPTED_FOR_IMPLEMENTATION`。R22対応でV1/V102/H2/entity/mapper、複合FK/self FK direct matrix、DATETIME(6) assignment、partial/repair metadata contract、operation ledger claim初期値・finished/failure/result状態行列、同一DBforward repair、P1-05のstarted_at付き不正claimと各初期値個別INSERTを同期したが、同一Headの実MySQL 0-skip証跡は未成立。`GATE-T066-HISTORY`は`TRACKED P2 / production release gate / 未実装・未受入`でS10 PASS/S12開始を阻害しない | S10 dispatch T066 / R19-P1-01 R22 schema follow-up | Base `230ce013a487ddd24175926344a2185ee29b1af4` → implementation `10097c3e` → docs同期後Headは`git rev-parse HEAD`で固定 | local target `0/0/3`（Docker daemon unavailable）、same-Head CI `1842/1/29/0` BUILD FAILURE、`git diff --check` PASS。T066 checkbox/S10/S12 status/ACTIVE/formal delivery変更なし | Docker付き同一HeadMySQL 0-skip、P1-01〜P1-05のR10 VERIFIED_CLOSED、G2 service/API/security→L1〜L3→Phase A/B→実在証跡→T066 L4が残る。S12はS10/S11双方PASSまでNOT READY |
