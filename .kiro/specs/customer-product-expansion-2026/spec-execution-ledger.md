...
| 8 | 1 | `crm-contact-opportunity` | T048〜T053 | **`IN PROGRESS`** | Round 4独立Review = **FAIL（P0=1/P1=4/P2=11/NOTE=7）**。P0-01は`4058d9b`で修正済み。P1-01〜04はコード/設計/定向testで対応済み、P2-01〜11はP2-08のbrowser確認をrelease QAへDEFERRED。migration実績は**V73/V74**、6列legacy-repair例外はdesignへ根拠固定 | S08 crm-contact-opportunity Round 4対応 | Base `8e5066a` → Head `eb5adc4`（main） | compile PASS。`MigrationScriptIntegrityTest` 25/25、CRM定向10/0/0、Contact service 3/0/0、customer controller 3/0/0、MessageBundle 4/4、Node 3/3、`git diff --check` PASS。Docker fresh/legacy/partial/repair/L4、roles_json MySQL往復、browser DemoはOPEN（環境不在） | Docker L4・実MySQL smoke・desktop/390px Demo完了までS07 approvalを解放しない |
