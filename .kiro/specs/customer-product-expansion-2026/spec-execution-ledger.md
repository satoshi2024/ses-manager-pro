...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 | `IN PROGRESS (Stage A: SpecHead)` | R3 Review FAIL (P0=0, P1=10 OPEN, P1-01 VERIFIED_CLOSED, P2=0)。Base `f8b81e77` に対し Stage A SpecHead (決定表・受入基準・Test Matrix・G4/G9確定) を策定・承認申請中。Stage B 実装は SpecHead 承認後 | S15 R3 是正 / Stage A SpecHead | Base `f8b81e77` | Direct 50/50, MySQL 5/5, Fast 2381/2381 PASS (コード実装済み、Spec 整合・深化のため Stage A 作成中) | Stage A SpecHead 承認 → Stage B 実装・検証 |
