...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060〜T065 PASS確定。**T066 M: 実装・L4全量（1824件・失敗0・skip 38=Docker gateのみ）完了、G2 gate未取得のためM PASS条件未達**・本番gate未達） | order PASS、G2 gate、V83実在/V82欠番 formal decision、S10=V84。R10 Round 9〜16の経緯（T061 F1 PASS・T062 F2 PASS・T063 A1 PASS・T064 B1 PASS、各fix deltaとVERIFIED_CLOSED）は旧行参照。R10 Round 17/18: T065 B2（`ca87e331`）を独立実行（全328クラス1817テスト、T065系10/0/0/0・回帰996/0/0/0）でPASS。新規R18-P1-01（worker snapshot列名不一致 age_over60_flag vs age_over_60_flag）はentityへ@TableField明示で解消（PR #68と同一fixが統合済み・AllMappersSchemaSweepTest 119/0/0/0）。R10 Round 18時点の他track起因2件（project.detail.desc・予約V99-V101衝突）はPR #68で統合担当が解消。**T066 M（`e6037f7c`）: worker snapshot由来項目の帳票全項目化（派遣元管理台帳へ性別・年齢・雇用期間・無期/60歳等、snapshot存在時のみ・sensitive）＋法務fixture3契約golden照合（派遣=MISSING系10件、準委任/請負=指示経路、BP階層4=TIER_EXCEEDED）＋L4全量**1824/0/0/0・skip 38（Docker gate）・BUILD SUCCESS**。履歴table由来項目はGATE-T066-HISTORYとして受入対象外に記録（design §3.1）。**G2 gate（COMPLIANCE_RESPONSIBLE runtime assignment・実actor承認event・外部専門家Review・PDF目視）未取得のためM PASS条件未達**。production release/apply authorizationなし | S10 dispatch T066 M（法務受入/回帰） | T065 B2 `ca87e331` → R18-P1-01 fix `69795ebc` → T066 M `e6037f7c`。V84実在・V85（exception_expires_at）追加 | L4全量 1824/0/0/0・skip 38（Docker gate、CIで実行されskip 0契約維持）・BUILD SUCCESS・`git diff --check` exit 0 | **G2 gate証跡取得（COMPLIANCE_RESPONSIBLE assignment・実actor承認event・外部専門家Review・PDF目視）→ R10がM PASS判定 → 本番release gate**。S12（staffing）はS10/S11双方PASS後（S11はT074 M PASS済み・release済み） |
