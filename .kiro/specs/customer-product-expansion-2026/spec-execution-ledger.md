...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T073完了・T071/T072独立Review **PASS・T072完結**、T073独立Review待ち、T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**。**R11 R5-P1-01/R5-P2-03 fix delta再Review: VERIFIED_CLOSED**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1）: freee/provider sync完結（法人別cursor・pull運用方針確定）**。**T073（B2 客先工数差異/通知、`5c34db26`）: read-only比較DTO（雇用勤怠vs契約工数・閾値480分既定）、理由確認はm_system_config JSON（DDLなし）、scope設計§5.3、日次バッチ通知（dedupe key冪等）、管理画面差異カード、新規2 class 13/0/0/0・指定回帰232/0/0/0 skip 0**。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` ほか → T072完了 `9be5e5c` → T073 `5c34db26` | R11 R5-P2-03 fix delta再Review: 独立実行 **219 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**。T073: 新規2 class **13/0/0/0 skip 0**＋指定回帰 **232/0/0/0 skip 0**、`git diff --check` PASS。P0=0/P1=0/P2=2（R2-P2-01/02、T074/M再評価）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T073独立Review→T074（M. 回帰/法務受入・L4全量）着手**。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
