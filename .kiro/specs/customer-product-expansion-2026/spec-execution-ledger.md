...
| 17 | 4 | `ai-feedback-learning` | T109〜T115 | **`READY`（T109 可開始）** | S16 CONDITIONAL PASS（Head `07ae187c`）。G10 は `decision-log` で blocking=no。T109 は mock/rule 既定・PII・metric の記録のみ（未正式選定の実 AI へ production code を変更しない）。F1（V108）は T110 から。着手時に Flyway latest を再確認し衝突なら後発を上へ採番 | T109 `0. G10/use case/PII/metric確定` 開始可 | S16 Head `07ae187c` | — | R17 PASSでroadmap完了 |
