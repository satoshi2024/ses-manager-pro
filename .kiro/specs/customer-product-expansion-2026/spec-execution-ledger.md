...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 / R4-T01〜R4-T08 | `IN PROGRESS (Stage A: SpecHead Revision 3)` | Base `f8b81e77` に対し Stage A SpecHead Revision 3 (取消決定表・Table 3同期、3段階リース45sフェンシング・CAS、V106.1 5形状・ロールバック、G4 PROVISIONAL運用契約、実在エンティティ組織導出・全Consumer、Tenant Timezone、入金1:1・fee計算式、Demo付きTask台帳) を確定・承認申請中。Stage B 実装は未着手・未解放 | S15 R4 是正 / Stage A SpecHead Revision 3 | Base `f8b81e77` | Direct 50/50, MySQL 5/5, Fast 2381/2381 PASS (コード実装済み、Spec 整合・深化のため Stage A 改訂中) | Stage A SpecHead 承認 → Stage B 実装・検証 |
