...
| 7 | 1 | `approval-workflow-internal-control` | T041〜T047 | **`IN PROGRESS`** | T041〜T046の実装記録は存在するが、B1/T046・M/T047は未完了。R1.2/R1.3のroute decision sourceをV79.1で実装し、APPLICANT_MANAGER（`t_user_organization.manager_user_id`）を含む5 sourceの追加source別境界・異常系回帰**47/0/0/0**を確認した。request→DB `route_snapshot_json`永続化→manager変更後のsnapshot/承認者不変も確認済み。R4-P1-01はV79.1の実MySQL fresh/legacy、履歴、checksum/FK/CHECK/index assertionに加え、専用回帰でpartial/repair/rollback、shared JDBCの複数JVM ShedLock/claim、commit前例外時実DB rollback、Webhook loopbackを確認済みだが、full application instance cron（必要な場合）、外部provider、5業務browser等の残存gateがあるため**OPEN / P1**を維持する。S07正式migrationは**V75/V76/V77/V78/V79**、R1.2/R1.3 patchは**V79.1**、S09以降はV80〜V88（V72永久欠番、out-of-orderなし） | S07 approval-workflow-internal-control 実装 | Review Base `5d228d2` → Review対象production Head `6680e7d81c7842262a2fd07c57fb9942e80573ce`（`main` / `origin/main` / `origin/HEAD`一致）。Base→Headは**22 commits / 212 files / +10459/-330**。production Head確認時worktreeはclean。Packet文書の**独立commitはなく4文書が現行Headに同梱済み**で、今回の文書同期、V79.1専用回帰追加、scheduler H2設定・回帰追加後は12ファイル（4資料、V79.1専用test/runbook、scheduler設定・H2回帰test、OperationalBoundary実MySQL回帰、Webhook loopback回帰）がdirtyとしてproduction Headと分離して管理する（commit/push禁止） | **NOT REVIEWABLE**。R4-REVIEW-01/03/04、R4-P1-01、B1/M、full application instance cron（必要な場合）、外部provider、5業務Browser Demoは未達。V79.1 partial/repair/rollbackは専用実MySQL回帰で、shared JDBCの複数JVM ShedLock/claim、commit前例外時rollback、Webhook loopback JSON POSTは`OperationalBoundaryMySqlIntegrationTest`/`WebhookNotifierLoopbackIntegrationTest`で**3/0/0/0**および**1/0/0/0**として確認済み。Chrome `150.0.7871.187`のlocalhost `/login` DOM smoke（1440x900/390x844、HTTP 200、login/form）は確認済みだが、5業務browser Demoや外部provider到達の代替にはしない。scheduler H2 lock warning回帰は**3/0/0/0**で確認済み。R4-REVIEW-02のみVERIFIED_CLOSED。R1.3対象**47/0/0/0**（RouteResolver 28、ApprovalAdministration 13、API 6）、static**35/0/0/0**、20クラスdirect consumer regression**150/0/0/0**。CI相当全量はDocker `29.6.1`/Node `v24.18.0`で**1471/0/0/0、BUILD SUCCESS、skip 0、script exit 0**を確認し、L4 zero-skippedは閉じた | Review対象production HeadのPacketを独立Reviewし、R4-P1-01とB1/Mの残存実環境DoDが完了するまでS09/Wave 2を解放しない |
