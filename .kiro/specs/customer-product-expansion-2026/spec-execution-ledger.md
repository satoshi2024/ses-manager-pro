...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS / FAIL`（T060〜T065 PASS。T066 schema phaseはR22-P1-01/P1-03のMySQL確認とP1-02/P1-04/P1-05のR10再Review待ち、G2実在証跡・Phase A/B・GATE-T066-HISTORY未達、T066 checkbox未完了） | order PASS、V84/V85実在。R19-P1-02/R19-P2-03/R20-P1-01 VERIFIED_CLOSED。R10がR21 canonical payload deltaを`ACCEPTED_FOR_IMPLEMENTATION`。R22対応でV1/V102/H2/entity/mapper、複合FK/self FK direct matrix、DATETIME(6) assignment、partial/repair metadata contract、operation ledger result NULL/hash/CAS boundaryを同期。`GATE-T066-HISTORY`は`TRACKED P2 / production release gate / 未実装・未受入`でS10 PASS/S12開始を阻害しない | S10 dispatch T066 / R19-P1-01 R22 schema follow-up | Base `6f159956f785cd5b6c2ddaf18de72a01cc90cde4` → implementation `9d1f1f7237f59e0847230f4b6990be735cd11ad2` → schema fix `57bb18eb` → packet tipはdocs同期後の`git rev-parse HEAD` | focused `7/0/0/0`、schema/consistency `161/0/0/0`、MySQL fresh/forward-repair/FK smokeはDocker不在skip。T066 checkbox/S10/S12 status/ACTIVE/formal delivery変更なし | R10独立ReviewでR22 P1残件、Docker MySQL 0-skip、partial/old-definition、result matrixを確認後、G2 service/API/security→L1〜L3→Phase A/B→実在証跡→T066 L4。S12はS10/S11双方PASSまでNOT READY |
