...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067/T068/T069/T070完了、T071〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。R2-P1-01はV82永久欠番、S11=V83、S10=V84の正式decisionでVERIFIED_CLOSED。R2-P1-02（休憩配賦spec）が独立VERIFIED_CLOSEDになるまでT071・次Waveは開始不可 | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067成果 `93c1ac6` → T068実装 `b327b1b` → T069 calculator実装 `d395797` → T070 workflow `cc7c15c` | R11 migration decision follow-upでR2-P1-01 VERIFIED_CLOSED、R2-P1-02 OPEN、P0=0/P1=1/P2=2。Flyway/MySQL指定回帰10/0/0/0、T068 H2 5/0/0/0、T069指定回帰70/0/0/0、T070指定回帰17/0/0/0。HR/法人別規程はATT-GATE-01〜06としてrelease gate | R2-P1-02の発注者による休憩区間保存またはbreakMinutes配賦規則のspec具体化→実装→境界回帰→独立VERIFIED_CLOSED後にT071開始。V83を変更・再適用せず、V82を補填しない |
...
- **Packet/current merged Head**: Packet同期commit（`git log -1 -- <path>`で解決）。Packet同期後の
  ...
  V80, V81, V83実在、S10=V84、S12=V85、S13=V86、S14=V87、S15=V88、S16=V89、S17=V90とする。V59/V72/V82は永久欠番。
  過去版の予約記載（V82〜V89）は履歴として保持するが、現行正本は上記のV83実在・V82欠番decisionによりS10=V84、S12〜S17=V85〜V90とする。
