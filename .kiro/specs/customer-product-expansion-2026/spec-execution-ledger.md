...
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T073完了・T071/T072/T073独立Review **PASS・完結**（R6-P2-01 VERIFIED_CLOSED、R6-NOTE-01 CLOSED）、T074（M）着手可） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**。**R11 R5-P1-01/R5-P2-03 fix delta再Review: VERIFIED_CLOSED**。**R11 Round 6 T073独立Review: PASS**。**R11 R6-P2-01 fix delta再Review: VERIFIED_CLOSED（SQL境界・閾値境界設計記録）**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1）: freee/provider sync完結**。**T073（B2 客先工数差異/通知）: read-only比較・理由確認（m_system_config JSON・R4.2不変）・日次バッチ通知（dedupe key冪等）・SQL境界scope**。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` ほか → T072完了 `9be5e5c` → T073 `5c34db26` → T073完了 `11398d9` → R6 fix `62e3d31` → R6転記 `ef4ce72` | R11 R6-P2-01 fix delta再Review: 独立実行 **234 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**、`AttendanceDiscrepancyServiceTest` **10/0/0/0**。R6-P2-01 VERIFIED_CLOSED、R6-NOTE-01 CLOSED、新規指摘なし。P0=0/P1=0/P2=2（R2-P2-01/02、T074/M再評価）/NOTE=4（cross-lane 3＋R6-NOTE-02）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T074（M. 回帰/法務受入・L4全量）着手可**。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
