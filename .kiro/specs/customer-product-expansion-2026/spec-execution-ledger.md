...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060 PASS、R4-P1-01 VERIFIED_CLOSED、T061/F1 FAIL・R5 P1×5 OPEN） | order PASS、G2確定済み。V83実在/V82欠番のformal decisionとS10正式migration V84をR10が確認。R10 Round 5はmapping 1対1、snapshot履歴、legacy/partial/repair、明示NULL、PII ownershipの5 P1を指摘。field-mapping §4、design §5.5/§6.2、tasks test matrixを先に改訂し、code fix前にR10再確認を受ける。production release/apply authorizationなし | S10 dispatch-outsourcing-compliance-ledger T061 F1 R5 fix plan | T061 review Base `856ab1faf09f07abcd7a5b34453a5037173ce553` → implementation Head `e7f7f19434e0e45d54888d6b468e9d8704c6056f` → docs fix/ledger sync Head（本commit） | T060 PASS、R4-P1-01 VERIFIED_CLOSED、R10 direct regression prior: MySQL 1/0/0/0（fresh only）、H2 1/0/0/0、SpecDispatch 9/0/0/0、Contract 48/0/0/0、Compliance API 1/0/0/0、skip 0。ただしR5 closure test matrix未実行 | R5 P1×5がVERIFIED_CLOSEDになるまでT061 checkboxを戻さず、T062/A1/B1/B2/production変更を開始しない。次のcode fixはfield-mapping/design/tasksのR10受理後に実施する |
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067/T068/T069/T070完了、T071〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。R2-P1-01はV82永久欠番、S11=V83、S10=V84の正式decisionでVERIFIED_CLOSED。R2-P1-02（方式A休憩区間）とR3-P2-01（breakMinutes不一致400）はR11 Round 3独立再Reviewで**VERIFIED_CLOSED**。V91実在に伴いS12〜S17の予約をV92〜V97へ繰り上げ済み。NOTE-R3-03（V1へのdispatch R5テーブル混入）は統合担当へ調整継続 | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067成果 `93c1ac6` → T068実装 `b327b1b` → T069 calculator実装 `d395797` → T070 workflow `cc7c15c` → V91方式A fix delta `5f362fc`/`b65996f`（committed Head=origin/main） | R11 Round 3独立再Review: **171/0/0/0 skip 0**（isolated worktree `b65996f`標準構成）、MySQL smoke 2/0/0/0、fresh全経路 2/0/0/0、`git diff --check` PASS。R2-P1-02/R3-P2-01 VERIFIED_CLOSED、P0=0/P1=0/P2=2/NOTE=1（NOTE-R3-06: dispatch新V84のtriggerが`FlywayMigrationSmokeTest` fresh経路をError 1419で破損・dispatch側修正待ち）。NOTE-R3-03はdispatch `b9b91f9`の整合commitで**解消**、NOTE-R3-04/05はFIXED（独立検証済み）。dispatch V84 R5 merge後treeの指定回帰171/0/0/0再確認済み。**T070方式A deltaのReview対話は収束完了**（次の独立ReviewはT071実装後）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T071開始可**（R2-P1-02 VERIFIED_CLOSEDで開始不可条件解消。P2×2はT071を止めない）。V83を変更・再適用せず、V82を補填しない。V91はS11方式A追補専用でS12〜S17はV92〜V97。NOTE-R3-03はdispatch V84 R5 merge時に統合担当が確認 |
...
- **Packet/current merged Head**: Packet同期commit（`git log -1 -- <path>`で解決）。Packet同期後の
  ...
  V80, V81, V83実在、S10=V84、S11=V83（方式A追補V91実在）、S12=V92、S13=V93、S14=V94、S15=V95、S16=V96、S17=V97とする。V59/V72/V82は永久欠番。
  過去版の予約記載（V82〜V89）は履歴として保持するが、現行正本は上記のV83実在・V82欠番decisionによりS10=V84、S11=V83＋追補V91、S12〜S17=V92〜V97とする。
