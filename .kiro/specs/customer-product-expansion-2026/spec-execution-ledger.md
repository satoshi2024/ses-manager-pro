...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060 PASS・T061 F1 PASS・T062 F2 PASS・**T063 A1 PASS**（R10 Round 13）。T064〜T065解放可・T066 M/本番gate未達） | order PASS、G2 gate、V83実在/V82欠番 formal decision、S10=V84。R10 Round 9: fix delta `34c68f7`を独立実行40/0/0/0 skip 0で確認し、R5-P1-01..05・R8-P0-01（休憩保存: break分整数列＋t_compliance_break_detail/trigger/manifest同期）・R8-P1-01（ALWAYS全clearable列＋F1NullClearMapperTestでfull DTO NULL/CAS 0行実測）・R8-P2-01（PATCH rejectはT063へ移管）・R8-NOTE-01（asOf effective interval解決）を全VERIFIED_CLOSED。R10 Round 10: T062 F2（`de08f2e4`）を独立実行で検証し、core実装・golden 12/12は良好もFAIL（新規P1×2: R10-P1-01=T061 PASS済みV84に誤字4行混入・`34c68f7`へ復元済み、R10-P1-02=null-profileでMISSING系ruleが全skipのfail-open・全field未入力として検知へ修正）。**R10 Round 11: fix `39f0384c`を独立実行（F2系30/0/0/0・F1系9/0/0/0・Integrity 27/0/0/0・skip 0）で確認し、T062 F2 PASS**。R10-P1-01（V84を34c68f7バイトへ完全復元・労動0・checksum整合）、R10-P1-02（null-profileを全field未入力としてMISSING検知・5 rule＋test正本化）をVERIFIED_CLOSED。R10 Round 12: T063 A1（`6d5e21f5`）をFAIL（R12-P1-01: SaveDto残留retention/legalHoldでUI保存400、R12-P2-01: contact顧客一致未検証）とし、fix `a6695026`（SaveDto 2列削除＋UI実data-key payload回帰test＋contact顧客一致チェック）を再提出。**R10 Round 13: fix `a6695026`を独立実行（T063系17/2/26・F2系30・F1系9・Integrity 27、skip 0）で確認し、T063 A1 PASS**。R12-P1-01（UI実data-key payloadでPUT 200を17件目testが回帰保証）、R12-P2-01（contact customer一致チェック＋400 test）をVERIFIED_CLOSED。残り失敗2件は他track起因（予約V99-V101衝突・project.detail.desc）でdispatch非関与・統合担当追跡。production release/apply authorizationなし | S10 dispatch T063 A1（R10 Round 13 PASS同期） | T063 A1（`6d5e21f5`）→ Round 12 fix（SaveDto 2列削除＋contact顧客一致＋回帰test 2件）`a6695026` → Round 13 PASS。V84実在 | 独立実行 123/0/0/0 skip 0のうちdispatch関連全PASS（T063系17/2/26＋F2系30＋F1系9＋Integrity 27）、`git diff --check` exit 0。残り2 failureはR10-P2-01他track起因 | **T064（B1）→ T065（B2）→ T066 M**（L4・runtime assignment/実actor承認event/外部専門家Review gate）。S12（staffing）はS10/S11双方PASS後（予約表衝突解消が前提）。S11側はdirty working tree（V98・不正文字）を先に解消のこと |
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T072完了・T071/T072独立Review **PASS**、**R5-P1-01 VERIFIED_CLOSED**、R5-P2-03 FIXED_BY_IMPLEMENTER（独立再Review待ち）、T073〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**、R5-P2-01/02 FIXED。**R11 R5-P1-01 fix delta再Review: VERIFIED_CLOSED、新規R5-P2-03（cursor×scope外、P2）OPEN→`3fce6d78`でFIXED_BY_IMPLEMENTER**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1 freee/provider sync）: AttendanceProvider interface＋mock/freee実装（apiGet/apiPost共通基盤、401 refresh 1回/429 backoff/timeout/冪等キー・相関ID/秘密非ログ）、冪等push・法人別cursor差分pull（tenant timezone正規化・R5-P2-03）・締め済み/承認済み月拒否→t_overtime_followup finding＋通知・read-only照合（一致/差異/該当なし＋差異サンプル）・caller scope（管理者全件/HR法人/manager組織、scope外skip）・CSV fallback、DDLなし**。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` → T072完了 `82b8612c` → R5 fix `c750fcb3` → R5転記 `ba5625f3` → R5-P1-01 fix `1582d96` → R5-P1-01転記 `a955aa3` → R5-P2-03 fix `3fce6d78` | R11 R5-P1-01 fix delta再Review: 独立実行 **218 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**。R5-P2-03 fix delta: sync系 **20/0/0/0**＋指定回帰 **219/0/0/0 skip 0**、`git diff --check` PASS。P0=0/P1=0/P2=3（R2-P2-01/02、R5-P2-03 FIXED_BY_IMPLEMENTER）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T073（B2. 客先工数差異/通知）着手可**。R5-P2-03の独立VERIFIED_CLOSED後が安全。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
