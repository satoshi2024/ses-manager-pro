...
| 9 | 2 | `order-acceptance-workflow` | T054〜T059 | **`FIX`（Round 5独立Review FAIL→Round6対応中）** | **R09 Round 5独立差分再Review = FAIL（P0=0/P1=1/P2=4/NOTE=0）**。prior evidence Base=2dbe83a / fix code Head=5158912 / evidence Head=4ee389d / **merged Head=8a50eb1（main/origin/main）**。R3-P1-04（初回submit・manager通知が対象月asOf scope未統一）をREOPEN。Round6対応で、`assertAllowedContractAsOf`追加・submit asOf化・通知のmanager解決をworkMonth asOf化、manager recipient直接test（同組織/異組織/無組織/重複）、V80 backfillのrepair-safe化（marker DMLが途中失敗でROLLBACKされることを実MySQLで実測し明示COMMIT追加）＋`FlywayV80RepairSmokeTest`、design V73残留修正、両ledger同期を実施 | S09 order-acceptance-workflow Round6対応 | Base `2dbe83a` → fix code Head `5158912` → evidence Head `4ee389d` → **merged Head `8a50eb1`**（main/origin/main、worktree clean） | R09 Round 5 = **FAIL**（独立Review）。Round6差分再ReviewでPASS確認が必要 | merge済みだがmergeは再発見P1を上書きしない。Round6対応（L4再実行含む）→差分再Review PASS後にS10 dispatch / S11 attendance（並行可・G2/G6決定済み）を解放 |
