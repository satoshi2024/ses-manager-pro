...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS`（T060 PASS・T061 F1 PASS・T062 F2 PASS・T063 A1 PASS・**T064 B1 PASS**（R10 Round 16）。T065解放可・T066 M/本番gate未達） | order PASS、G2 gate、V83実在/V82欠番 formal decision、S10=V84。R10 Round 9: fix delta `34c68f7`を独立実行40/0/0/0 skip 0で確認し、R5-P1-01..05・R8-P0-01（休憩保存: break分整数列＋t_compliance_break_detail/trigger/manifest同期）・R8-P1-01（ALWAYS全clearable列＋F1NullClearMapperTestでfull DTO NULL/CAS 0行実測）・R8-P2-01（PATCH rejectはT063へ移管）・R8-NOTE-01（asOf effective interval解決）を全VERIFIED_CLOSED。R10 Round 10: T062 F2（`de08f2e4`）を独立実行で検証し、core実装・golden 12/12は良好もFAIL（新規P1×2: R10-P1-01=T061 PASS済みV84に誤字4行混入・`34c68f7`へ復元済み、R10-P1-02=null-profileでMISSING系ruleが全skipのfail-open・全field未入力として検知へ修正）。**R10 Round 11: fix `39f0384c`を独立実行（F2系30/0/0/0・F1系9/0/0/0・Integrity 27/0/0/0・skip 0）で確認し、T062 F2 PASS**。R10-P1-01（V84を34c68f7バイトへ完全復元・労動0・checksum整合）、R10-P1-02（null-profileを全field未入力としてMISSING検知・5 rule＋test正本化）をVERIFIED_CLOSED。R10 Round 12: T063 A1（`6d5e21f5`）をFAIL（R12-P1-01: SaveDto残留retention/legalHoldでUI保存400、R12-P2-01: contact顧客一致未検証）とし、fix `a6695026`（SaveDto 2列削除＋UI実data-key payload回帰test＋contact顧客一致チェック）を再提出。**R10 Round 13: fix `a6695026`を独立実行（T063系17/2/26・F2系30・F1系9・Integrity 27、skip 0）で確認し、T063 A1 PASS**。R12-P1-01（UI実data-key payloadでPUT 200を17件目testが回帰保証）、R12-P2-01（contact customer一致チェック＋400 test）をVERIFIED_CLOSED。R10 Round 14: T064 B1（`84101461`）をFAIL（R15-P1-01: downloadがviewer roleで再maskされない・mask済PDF正本化、P1-02: party_*未投入＋label不一致、P1-03: mapping項目未出力、P1-04: 営業403が未文書化逸脱）とし、fix `ca47e7f1`（generate常にFULL正本＋download viewer再レンダリング＋company系config→party snapshot化＋generator拡充＋営業masked一覧/download可・design §3.1へscope明記）を再提出。**R10 Round 16: fix `ca47e7f1`を独立実行（B1系13・T063系45・F2系30・F1系9・Integrity 27、skip 0）で確認し、T064 B1 PASS**。R15-P1-01（cross-role byte差test実測）、P1-02（party実フロー投入）、P1-03（typed列由来項目追加・履歴/worker由来はT066で全項目化とdesign §3.1明記）、P1-04（営業動線）をVERIFIED_CLOSED。残り失敗2件は他track起因（予約V99-V101衝突・project.detail.desc）でdispatch非関与・統合担当追跡。production release/apply authorizationなし | S10 dispatch T064 B1（R10 Round 16 PASS同期） | T064 B1（`84101461`）→ Round 15 fix（download再mask・party snapshot化・generator拡充・営業動線）`ca47e7f1` → Round 16 PASS。V84実在 | 独立実行 136/0/0/0 skip 0のうちdispatch関連全PASS（B1系13＋T063系45＋F2系30＋F1系9＋Integrity 27）、`git diff --check` exit 0。残り2 failureはR10-P2-01他track起因 | **T065（B2）→ T066 M**（L4・履歴/worker由来の帳票全項目化・runtime assignment/実actor承認event/外部専門家Review gate）。S12（staffing）はS10/S11双方PASS後（予約表衝突解消が前提）。S11側はdirty working tree（V98・不正文字）を先に解消のこと |
| 11 | 2 | `attendance-leave-overtime-compliance` | T067〜T074 | `IN PROGRESS`（T067〜T072完了・T071/T072独立Review **PASS・T072完結**、**R5-P1-01/02/03 VERIFIED_CLOSED/FIXED**、T073〜T074未着手） | order PASS、G6確定済み。dispatchと並行可。**R11 Round 4 fix delta再Review（`85ca62ba`）: R4-P1-01/R4-P2-01 VERIFIED_CLOSED**。**R11 Round 5 T072独立Review: PASS**。**R11 R5-P1-01 fix delta再Review: VERIFIED_CLOSED**。**R11 R5-P2-03 fix delta再Review: VERIFIED_CLOSED（法人別cursor・pull運用方針確定）**。V91（方式A）・V98（休暇残数台帳、発注者割当）実在。**T072（B1 freee/provider sync）: AttendanceProvider interface＋mock/freee実装（apiGet/apiPost共通基盤、401 refresh 1回/429 backoff/timeout/冪等キー・相関ID/秘密非ログ）、冪等push・法人別cursor差分pull（tenant timezone正規化）・締め済み/承認済み月拒否→t_overtime_followup finding＋通知・read-only照合（一致/差異/該当なし＋差異サンプル）・caller scope（engineer scope＋LE scope二重確認）・CSV fallback、DDLなし**。cross-lane NOTE×3: NOTE-R3-06（dispatch V84 fresh経路）、NOTE-R4-03（`project.detail.desc`欠落、scale-300）、NOTE-R4-04（V101予約衝突、scale-300採番）は統合担当OPEN | S11 attendance-leave-overtime-compliance 実装 | Base `5e29f39` → T067 `93c1ac6` → T068 `b327b1b` → T069 `d395797` → T070 `cc7c15c` → V91方式A `5f362fc`/`b65996f` → T071休暇 `7981e5c`/`6d99658a` → R4 fix delta `85ca62ba` → T072 `840539da` → T072完了 `82b8612c` → R5 fix `c750fcb3` → R5転記 `ba5625f3` → R5-P1-01 fix `1582d96` → R5-P1-01転記 `a955aa3` → R5-P2-03 fix `3fce6d78` → R5-P2-03転記 `9be5e5c` | R11 R5-P2-03 fix delta再Review: 独立実行 **219 tests / 1 failure（既知NOTE-R4-03のみ）/ 0 errors / 0 skipped**、`AttendanceSyncServiceTest` **13/0/0/0**。R5-P2-03 VERIFIED_CLOSED、新規指摘なし。P0=0/P1=0/P2=2（R2-P2-01/02、T074/M再評価）/NOTE=3（cross-lane）。HR/法人別規程はATT-GATE-01〜06としてrelease gate | **T073（B2. 客先工数差異/通知）着手可**。pull運用方針（法人別cursor）確定済み。V83/V91/V98を変更・再適用せず、V82を補填しない。V101はscale-300実在のためS13/S14予約の再繰上げは統合担当（NOTE-R4-04） |
