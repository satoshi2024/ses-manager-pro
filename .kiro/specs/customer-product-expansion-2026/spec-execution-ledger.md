...
## S10 dispatch R21 canonical payload受理後 / T066 G2実装フェーズ1 update

R10の独立ReviewでR21 canonical payload deltaは`PASS / ACCEPTED_FOR_IMPLEMENTATION`。R21残存issueはVERIFIED_CLOSEDとして同期し、T066/R19-P1-01実装フェーズ1でV102、V1、H2、G2 entity/mapper、delivery immutable source FK、V102専用MySQL smokeを実装・追加した。定向H2/mapper/migration整合はPASS、実MySQL smokeはDocker不在でskip。S10は`IN PROGRESS / FAIL`、T066 checkbox未完了、ACTIVE化・本番generate/delivery・production authorization禁止、S12は`NOT READY`を維持する。G2 service/API/UI/security、L1〜L3、Phase A/B、実在証跡、T066 L4は後続工程であり、R10の各フェーズReviewなしに完了扱いにしない。
...
| 10 | 2 | `dispatch-outsourcing-compliance-ledger` | T060〜T066 | `IN PROGRESS / FAIL`（T060〜T065 PASS。R10独立ReviewでR22-P1-05は`FIXED_BY_IMPLEMENTER / MYSQL_VERIFICATION_PENDING`、P1-02/P1-04も同pending、P1-01/P1-03は`OPEN / MYSQL_VERIFICATION_PENDING`。G2実在証跡・Phase A/B・GATE-T066-HISTORY未達、T066 checkbox未完了） | order PASS、V84/V85実在。R19-P1-02/R19-P2-03/R20-P1-01 VERIFIED_CLOSED。R10がR21 canonical payload deltaを`ACCEPTED_FOR_IMPLEMENTATION`。R22対応でV1/V102/H2/entity/mapper、複合FK/self FK direct matrix、DATETIME(6) assignment、partial/repair metadata contract、operation ledger claim初期値・finished/failure/result状態行列、同一DBforward repair、P1-05のstarted_at付き不正claimと各初期値個別INSERTを同期。`GATE-T066-HISTORY`は`TRACKED P2 / production release gate / 未実装・未受入`でS10 PASS/S12開始を阻害しない | S10 dispatch T066 / R19-P1-01 R22 schema follow-up | Base `230ce013a487ddd24175926344a2185ee29b1af4` → implementation `10097c3e` → docs同期後Headは`git rev-parse HEAD`で固定 | current focused `5/0/0/0`（H2 3、mapper 2）、schema/consistency `161/0/0/0`、Docker MySQL 1 smokeは環境skip。T066 checkbox/S10/S12 status/ACTIVE/formal delivery変更なし | Docker付きMySQL 0-skip、P1-01〜P1-05のVERIFIED_CLOSED条件、G2 service/API/security→L1〜L3→Phase A/B→実在証跡→T066 L4が残る。S12はS10/S11双方PASSまでNOT READY |
...
| 12 | 2 | `staffing-capacity-planning` | T075〜T080 | `NOT READY`（**S11 PASS済み・S10 PASS待ち**。S10/S11双方PASSまで着手不可。着手時はS12専用派工対話・V103採番を再確認） | dispatch/attendance PASS後S12 |  |  |  | R12 PASSでWave 2完了 |
