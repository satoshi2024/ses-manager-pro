...
| 15 | 3 | `accounting-payment-integration` | T094〜T101 / R4-T01〜R4-T08 | `IN PROGRESS (Stage A: SpecHead Revision 6)` | Base `f8b81e77` に対し Stage A SpecHead Revision 6 (新UNIQUE削除→退避行UPDATE復元→旧UNIQUE復元→全11追加列個別DROP→backup削除の厳格順序Rollback SQL・Flyway repair・MySQL回帰受入定義、OAuth敗者RETRYABLE(5s)、freee固定contract fixture、経費履歴asOf、Tenant別timezone) を確定・承認申請中。Stage B 実装は未着手・未解放 | S15 R4 是正 / Stage A SpecHead Revision 6 | Base `f8b81e77` | Direct 50/50, MySQL 5/5, Fast 2381/2381 PASS (コード実装済み、Spec 整合・深化のため Stage A 改訂中) | Stage A SpecHead 承認 → Stage B 実装・検証 |
