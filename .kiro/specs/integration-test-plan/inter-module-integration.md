...
| ITb-08 (Edge) | タイムゾーン境界 → 勤怠・契約連動 | UTC時間帯等の別タイムゾーン環境のブラウザから、月末（31日など）の境界をまたぐタイミングで打刻・勤怠提出を行う。 | JVMの `Asia/Tokyo` 設定と `DateUtils` により、JST基準で正確に当月度として処理され、翌月への誤計上が発生しないこと。 |
