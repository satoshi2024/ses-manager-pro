...
| `accounting-payment-integration-R1-P1-03` | P1 | **VERIFIED_CLOSED** | R1.4, R4.1, R4.3 | design §4 | 敗者ノードの最終動作定義（3回待機後 `TokenRefreshInProgressException` 送出と Job の `RETRYABLE(5s)` 遷移）。401 発生時の `observedTokenVersion` 引数渡しによる二重更新防止。Deal 作成タイムアウト未知結果の 50 ページ pagination 走査 (`verifyDealCreatedByRefNumber`) | `R4-T02` (`FreeeAccountingProviderTest#forceRefreshToken_multiNode_3StepLease_httpOutsideTx`) |
...
| `accounting-payment-integration-R4-P1-01` | P1 | **VERIFIED_CLOSED** | handbook §8, §12 | tasks §1, §2 | 歴史的タスク（T094〜T101, `[x]`）と Stage B 是正タスク（R4-T01〜R4-T08, 全て `[x]` かつ詳細 Demo / CONDITIONAL PASS 条件定義付き）を明確に分離し、台帳整合と accounting 専用 Browser Demo を完備 | `tasks.md`, `review-ledger.md`, `spec-execution-ledger.md`, `AccountingIntegrationBrowserDemoTest` |
| `accounting-payment-integration-R1-P1-04` | P1 | **VERIFIED_CLOSED** | R1.1 | design §1.1, §1.2 | V106 を `67f0f7aa` に復元。新 UNIQUE 先行削除 → 退避行 UPDATE 復元 → 旧 UNIQUE 復元 → 全 11 追加列の独立存在判定 DROP → backup 後置削除。direct MySQL regression 契約（NULL 法人重複 2 件 apply→rollback→全行復元、任意 partial 形状、V106 形状完全一致、repair→再適用）を確定 | `R4-T01` (`IntegrationConnectionAndJobTest#migration_rollback_partialSafeAllShapes`) |
| `accounting-payment-integration-R1-P1-07` | P1 | **VERIFIED_CLOSED** | R2.2, R2.3 | design §1.1, §3.2 | Worker は業務テーブルを再読込せず `t_integration_job.payload_snapshot` 列から直接 deserialize して送信。取消も snapshot 内の deal ID / reason を使用 | `R4-T05` (`SalesInvoiceIntegrationTest#cancelJob_executesStrictlyFromSnapshot`) |
| `accounting-payment-integration-R1-P1-10` | P1 | **VERIFIED_CLOSED** | R4.5 | design §7 | 生レスポンス・生例外の完全遮断。定型エラーコード (`VALIDATION_ERROR`, `UNAUTHORIZED`, `PLAN_LIMITATION`, `RATE_LIMITED`, `SERVER_ERROR`, `NETWORK_ERROR`) と局所化キーのみ保存・ログ出力 | `R4-T03` (`FreeeAccountingProviderTest#errorHandling_sanitizedCodesAndNoPii`) |
| `accounting-payment-integration-R1-P1-11` | P1 | **VERIFIED_CLOSED** | R6 | design §8 | `messages*.properties` (ja/en/zh/ko) を単一翻訳源とし、HTML / JS の全可視文言を `t(key)` 化。取消理由を機械可読コードで送信・保存 | `R4-T04` (`AccountingIntegrationApiAndPageTest#i18n_fourLanguages_stableReasonCodes`) |
