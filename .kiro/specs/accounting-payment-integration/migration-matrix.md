...
S15のrepair専用migrationはV105.4とV106.2であり、S16のV107は使用しない。

| 形状 | 開始状態 | 実行経路 | 期待結果 | 直接検証 |
|---|---|---|---|---|
| Fresh | consolidated V1 | V105.4(no-op) → V106 → 旧V106.1 → V106.2 | 最終UNIQUEが`external_company_key`を含む。NULL companyのsoft-delete再作成可 | fresh latest、index/column assert |
| Historical legacy | V105.3相当。`m_integration_connection`未作成、`t_freee_connection`に同一productでcompany_id違い2件 | V105.4 preflight → V106 → 旧V106.1 → V106.2 | V106で衝突せず、退避行をV106.2が復元し、2 companyがactive | historical fixtureで全履歴とcompany row再読 |
...
- V105.4は`m_integration_connection`が既に存在するfresh/baseline経路で業務データを変更しない。
