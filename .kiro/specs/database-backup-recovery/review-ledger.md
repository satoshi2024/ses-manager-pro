...
| HFP-03-003 | REVIEWABLE | NOT_REVIEWED | `ops/backup/backup-full.sh`（legacy 置換）, `lib/manifest.sh`, `lib/metadata.sh`, `tests/full-backup-test.sh`, `tests/fixtures/bin/{mysqldump,restic}`（fixture）, `README.md` | full-backup-test.sh 35 assert 全 PASS（R1 修正: 失敗時 quiesce 解放・coordinate 書き出し）（coordinate parse、quiesce/restic 失敗、symlink、1byte 破損、extra file、metadata 後書き、forget 呼出 0）。shellcheck exit 0。隔離 Demo: 実 MySQL+restic で full→restore verify→marker/table count/uploads hash 照合→metadata 改変で verify 失敗 | `target/backup-recovery-evidence/20260814-hfp03/HFP-03-003/`（backup-full-result.json=`7b3119d0...` restore-verify.txt=`c7c28518...` tamper-verify.txt=`e6d26ae5...` manifest-sha.txt=`d63c13d4...` engineer-count-before.txt=`53c234e5...`） | 発見: 背景 DDL session が lock fd/pipe を握る問題を fd 固定化+閉鎖で解決（review 対象）。metadata 内 status は PENDING で upload し、restic tag status=valid 昇格を selector が参照する |
| HFP-03-004 | REVIEWABLE | NOT_REVIEWED | `ops/backup/archive-binlog.sh`（legacy 置換）, `snapshot-binlog.sh`（legacy 置換）, `create-checkpoint.sh`（新規）, `lib/binlog.sh`（新規）, `tests/binlog-checkpoint-test.sh`（新規）, `tests/fixtures/bin/{mysql,mysqlbinlog}`（SHOW/FLUSH/archive/verify fixture 拡張）, `lib/common.sh`（restic::ensure_repository）, `docker-compose.yml` | binlog-checkpoint-test.sh 51 assert 全 PASS（R1 修正: 初回 coordinate 必須）（archive resume/UUID/server-id/欠番/checksum/truncated/active 除外/rotation 2 回）。shellcheck exit 0。隔離 Demo: 実 MySQL で --stop-never archive → checkpoint（実 FLUSH/rotation/uploads snapshot）→ 前 marker のみ DB/uploads に存在・解除後 marker は snapshot に不在、closed binlog 3 本 | `target/backup-recovery-evidence/20260814-hfp03/HFP-03-004/`（checkpoint-result.json=`e84f774a...` checkpoint-index-values.txt=`17132852...` marker-files-restored.txt=`d7967249...` immutable-files.txt=`d7f4ac05...` raw-files.txt=`c7f748fc...` binlogs-before.txt=`728a7677...` binlogs-after.txt=`cd08104b...`） | 実装中発見: (1) 背景 DDL session の fd 継承（前 task で対応） (2) checkpoint→snapshot-binlog の lock 自己デッドロックを --no-lock で解決 (3) $(...) 内 init のスコープ問題を main 前置きで解決。いずれも review 対象 |
...
| HFP-03-RQ-008 | HFP-03-AC-008-02 | HFP-03-010 | `retention.sh` の RETENTION_ROLE 検査（retention/admin のみ。writer=backup は dry-run も拒否） | retention-test.sh: writer_denied / role_absent | Demo: role=backup → rc=1 | NOT_REVIEWED |
| HFP-03-RQ-008 | HFP-03-AC-008-03 | HFP-03-010 | `rotate-key.sh`（旧・新の両キーで restore verify → 成功時のみ atomic 切替）+ `runbooks/key-rotation.md` | retention-test.sh: key_rotation（新キー失敗 → 切替なし / 両成功 → 切替） | — | NOT_REVIEWED |
...
| HFP-03-RQ-009 | HFP-03-AC-009-01 | HFP-03-010 | `lib/dependency-graph.sh` + `retention.sh`（PITR window 全保持 + 日次/週次/月次代表。各保持 checkpoint の full/binlog/uploads チェーンを保持） | retention-test.sh: window_all_kept / daily_weekly_rep / old_chain_deletable | Demo: A/B/C 保持・D のみ削除 | NOT_REVIEWED |
| HFP-03-RQ-009 | HFP-03-AC-009-02 | HFP-03-010 | maintenance lock（bounded timeout）下で forget --prune。取得失敗 → 非 0 + alert 文言 | retention-test.sh: prune_race | — | NOT_REVIEWED |
| HFP-03-RQ-009 | HFP-03-AC-009-03 | HFP-03-010 | apply は dry-run report を再計算して一致確認（AC-009-03）。依存が残る snapshot は PITR_AVAILABLE 表示 | retention-test.sh: apply_report_mismatch / missing_dependency（pitr_available=false） | Demo: dry-run == apply（report SHA bind） | NOT_REVIEWED |
...
| HFP-03-RQ-011 | HFP-03-AC-011-01 | HFP-03-008,011,012 | `validate-restore.sh` + `lib/validate-db.sh`（Flyway failed 0 / 最新 version 一致 / CHECK TABLE / critical counts / marker before 存在・after 不在） | restore-validation-test.sh: flyway_failed / flyway_version / count_mismatch / marker_after。integration suite（実 MySQL PITR）: before=1/after=0 | Demo: validate → READY_FOR_CUTOVER / integration: SUCCESS | NOT_REVIEWED |
| HFP-03-RQ-011 | HFP-03-AC-011-02 | HFP-03-011 | integration suite（実 MySQL の negative test: gap 検出 / target 前後 / timezone は unit（restore-plan-test.sh timezone_independent）+ restore-validation-test.sh の negative 一式で担保） | unit: restore-plan-test.sh / restore-validation-test.sh / retention-test.sh（prune race） | — | NOT_REVIEWED |
...
|---|---|---|---|---|---|---|---|
| HFP-03-R1-P0-01 | P0 | RQ-007/012 | `restore.sh`（import 前の GTID リセット欠如） | 同一 target への 2 回目 restore（drill の DROP/CREATE 再利用パターン）で replay が沈黙スキップされ DML 欠落（review 実測） | 再 restore 時の復元正当性が保証されない | import 前に `RESET MASTER`（gtid_executed リセット）。restore-svc に RELOAD 権限を追加。integration に中間 DML（marker-mid）の replay 照合を追加 | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-01 | P1 | RQ-006 | `lib/approval.sh`（単一 APPROVAL_PUBKEY / 同一鍵 2 名） | 同一 key で別名 actor 2 件が二者承認を充足（review 実測 ACCEPTED） | 1 名で承認が成立 | 2 件の claim は異なる検証鍵（鍵内容）で署名されていることを必須化 | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-02 | P1 | RQ-006 | `lib/plan.sh`（content_for_sha が valid_until/plan_id を除外） | valid_until_utc 改変が plan SHA 検出を素通り（期限バイパス実測） | 期限延長・plan_id 改変の未検出 | content_for_sha は created_at のみ除外（valid_until/plan_id を含める）。plan_id 導出は別関数（timezone 非依存を維持） | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-03 | P1 | RQ-002 | `backup-full.sh`（trap が cleanup を呼ばず quiesce 未解放） | backup-full 失敗時に DDL lock session が生存継続（review 実測） | 書込み静止が解除されず DDL が凍結継続 | EXIT trap を `backup_full::cleanup; rm -rf work` に連結（quiesce::release を必ず実行） | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-04 | P1 | RQ-011 | `validate-restore.sh`（find\|head -1 の任意 checkpoint 照合） | plan が参照しない checkpoint と照合し false negative（review 実測） | 検証が plan の checkpoint と無関係になり得る | plan の `effective_checkpoint.index` の file のみを読む（無ければ fail） | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-05 | P1 | RQ-008 | `restore.sh`/`validate-restore.sh`/`cutover.sh`（TARGET_TLS_MODE を無検証で使用） | TARGET_TLS_MODE=DISABLED で平文 restore 成功（review 実測） | 秘密が平文で復元経路に流れる | VERIFY_CA / VERIFY_IDENTITY 以外は拒否 + TARGET_SSL_CAPATH 必須 | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-06 | P1 | RQ-004 | `archive-binlog.sh`（初回起動の coordinate 未要求）+ `backup-full.sh`（未書き出し） | 初回起動で先行 binlog が黙って未アーカイブ化、health も検出不能（review 実測） | PITR 窓の実効縮小が無検出 | backup-full が FULL_COORDINATE_FILE を書き出し（必須化）、archiver は初回起動に同 file を必須化（欠如で fail） | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P1-07 | P1 | RQ-012 | `rollback-cutover.sh`（state file 欠如を initial 扱い） | write-enabled 後に state file 削除で rollback 許可（review 実測 rc=0） | 新規 transaction を失う rollback が可能 | state file 欠如は unknown として rollback 拒否 | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P2-01 | P2 | RQ-007 | `restore.sh`（binlog SHA 未照合） | — | 破損 binlog を replay し得る | replay 前に plan の sha256 と照合（不一致で fail） | FIXED_BY_IMPLEMENTER |
| HFP-03-R1-P2-02 | P2 | RQ-008 | demo/integration の evidence に署名秘密鍵・password | — | 秘密の混在 | 署名後に秘密鍵を削除、secret scan に PRIVATE KEY を追加、既存 evidence の priv*.pem を除去 | FIXED_BY_IMPLEMENTER |
...
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/dry-run.json` | `aafbb76d31673e99bd9533a2669f8fba88e5e73eaf3bde27dba65f549282bf46` | HFP-03-010 Demo | secret grep 0 | gitignore 対象 |
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/apply-result.json` | `1cd96ee92191ce21504f8b6dac76125a6882ec581e6a526a9f503a7cfa084247` | 同上 | 同上 | 同上 |
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/snapshots-after.txt` | `a79cf72827b11f15a2e4b382e1c678f1b9a2c4d9bb2be7772a9c8588b61e1ee3` | 同上 | 同上 | 同上 |
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/plan-result.txt` | `3ce87833711f8807e0dda1c6412842abf103bd8155a97491c1a0454422cfb1ce` | 同上 | 同上 | 同上 |
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/restore-verify.txt` | `8c1561a674b682bc6cec7509fce337faa1b4a43d8e302e782e6e8fe6728e7e5a` | 同上 | 同上 | 同上 |
| `target/backup-recovery-evidence/20260814-hfp03/HFP-03-010/writer-denied.txt` | `8169bd4aced34623a9f5b959f99c006c82258f343faf86a8f67fe943f77d568f` | 同上 | 同上 | 同上 |
| `ops/backup/tests/.integration-work/evidence/integration-summary.json` | `76b909098fc7cf6820f7a9f4cce0f45e730d6d8155bbf0586b9560b42caafeaa` | HFP-03-011 integration suite | secret grep 0 | gitignore 対象 |
| `ops/backup/tests/.integration-work/evidence/validate.json` | `354d90f6bd0eda385900b241299608bdd940a98a77ee3aee137f92fbf8a241cc` | 同上 | 同上 | 同上 |
...
| `ops/backup/tests/.integration-work/evidence/integration-summary.json`（R1 fix 後） | `2a926abc11845a83fd5d348aa7160bbac6103510d5acd84a7a36d100baa73b1d` | R1 修正後の integration + drill（mid_dml_replayed=1） | secret grep 0（PRIVATE KEY 含む） | gitignore 対象 |
| `ops/backup/tests/.integration-work/evidence/validate.json`（R1 fix 後） | `19472cd512a3af68023b8794a8ea05d88713f03beaf2e8a45b025b8fe3692b1e` | 同上 | 同上 | 同上 |
...
| 2026-08-16T10:00Z | 独立 Reviewer（最終 Review） | **PASS**（merge 済み head `5c639d79` の独立 Review） | OPEN finding 0（P0=0 / P1=0 / 未管理 acceptance=0）。HFP-03-RF-P1-01 は VERIFIED_CLOSED | fix delta `3c908d61..5c639d79` は `.gitignore`（marker 3 行削除）+ docs のみ。`git check-ignore -v` で 5 pattern 全て機能。main の backup file は reviewed head `1e34f47e` と完全一致。main 直接回帰: unit 430 assert / integration SUCCESS（mid_dml_replayed=1 / drill rpo=60s / secret scan 0）。GATE-01〜05/07/08 PASS |
| 2026-08-16T10:00Z | 独立 Reviewer | production-ready 判定は保留 | **production-ready とは判定しない** | HFP-03-PROD-001〜008（特に PROD-004 deployment/cutover provider、PROD-006 承認 verifier、PROD-007 代表 profile）が発注者により確定し release gate **HFP-G03 が CLOSED** になるまで production への接続・復元・cutover を開始しない。確定後は baseline.md §4 の再実行手順に従う |
