...
| **CR-06** | NF-09対象テストの完全性（専用Fast 69件 + MySQL実コンテナ 3件） | NF-09対象14テストクラス (69メソッド) + MySQL 1クラス (3メソッド) | NF-09専用Fast: **69/69 tests PASS (0 failure, 0 error, 0 skipped)**<br>MySQL Gate: `mvn test -Pmysql-tests -Dtest=AssetMySqlIntegrationTest` (**3/3 PASS, 0 failure, 0 error, 0 skipped**)。V131追随の既存migration smoke 4件もPASS。`MySqlTestShardInventoryTest` PASS & `AssetMySqlIntegrationTest` 登録。固定seed `27838638095700` の全体比較は Base `3060 tests / 2 failures / 16 errors / 0 skipped`、R6実装Head `3123 tests / 2 failures / 11 errors / 0 skipped`。Head初回のfeature起因2件は修正済みで、残存はBaseにも存在する既存/環境側テスト。全体Fast gateは **未PASS** | **NF-09対象PASS / 全体gate未PASS（Base比較済み）** |
...
5. **移管/返却/紛失/廃棄履歴を上書きしない (AS-R1.4)**:
   ...
   - `AssetAssignmentServiceImpl.removeById`、`ExternalAccountService.softDeleteAccount`、`LicenseService.softDeleteAssignment` は状態にかかわらず既存履歴の論理削除を拒否し、`RETURNED`/`REVOKED`/`RELEASED`を保持する。
...
- R6実装Head `f37501fc79ebfed006887a8c920f3c7c2c5bc709` の同条件: `tests=3123, failures=2, errors=11, skipped=0`。追加されたR6対象テストは全件成功し、残存8クラスはR5/Base比較と同じ既存・環境側クラス。
- R5時点Headの残存クラスはBaseにも存在する。初回Headで追加された `TransactionalRollbackForAuditTest`（資産系更新@TransactionalのrollbackFor不足）と `MyAssetApiControllerTest`（新規scope fixtureによるH2共有DB汚染）はfeature起因として修正した。R5時点の関連12/12、NF-09 66/66、MySQL 3/3に加え、R6ではNF-09 69/69、MySQL 3/3、V131追随migration smoke 4/4をPASSした。
- したがってNF-09対象はPASS、リポジトリ全体Fast gateは未PASSであり、独立Reviewでこの扱いを確認する。

- **検証対象実装remote Head**: R6実装commit `f37501fc79ebfed006887a8c920f3c7c2c5bc709` はpush済み。文書・テスト追随commit後の最終Review HeadはReview開始時に同コマンドで再固定する。
