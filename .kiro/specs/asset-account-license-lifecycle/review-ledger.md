...
| **AS-R3** | 実地棚卸し差異照合（確定後更新拒否）・紛失インシデント追跡・NF-01退社 3大blocker連携 | `t_asset_inventory_run`, `t_asset_inventory_item`, `t_asset_lost_incident`, `t_asset_offboarding_waiver`, `AssetInventoryService`, `AssetLostIncidentService`, `AssetOffboardingService`, `ResignationGateChecker`, `AssetAlertService` | `AssetServiceTest.testInventoryRunFlow`<br>`AssetServiceTest.testLostIncidentLedgerAndEmergencyAlert`<br>`AssetApiControllerTest.testLostIncidentApiFlow`<br>`AssetBoundaryAndLifecycleIntegrationTest.testInventoryDisallowUpdateAndDoubleComplete`<br>`AssetBoundaryAndLifecycleIntegrationTest.testInventoryConcurrentCompletionSingleWinner`<br>`ResignationGateFailureDrillTest`（9/9）<br>`AssetBoundaryAndLifecycleIntegrationTest.testOffboardingThreeBlockers`<br>`AssetOffboardingServiceTest` | **PASS** |
...
| **CR-02** | 状態・競合・冪等性 (状態遷移表 / CAS / FOR UPDATE 排他 / 二重失効安全 / Provider Recovery / UNKNOWN分類) | `AssetStatusPolicy`, `AssetServiceImpl`, `AssetAssignmentServiceImpl`, `AssetInventoryServiceImpl`, `LicenseServiceImpl`, `ExternalAccountProviderClient`, `ExternalAccountServiceImpl` | `AssetServiceTest.testAssetStatusTransitionGuard` (許可遷移・専用貸与入口)<br>`AssetBoundaryAndLifecycleIntegrationTest.testActualReturnDateRangeIsEnforced` (実返却日の開始日〜当日制約)<br>`AssetAssignmentConcurrencyTest` (並行4スレッド排他)<br>`AssetBoundaryAndLifecycleIntegrationTest.testConcurrentReturnAndWaiveSingleTerminalEvent` (返却/免除競合、終端event 1件)<br>`AssetBoundaryAndLifecycleIntegrationTest.testLicenseConcurrentAllocationWithCas` (並行4スレッド2席CAS排他)<br>`AssetBoundaryAndLifecycleIntegrationTest.testLicenseConcurrentReleaseDecrementsOnce` (二重解放で席数1回減算)<br>`AssetBoundaryAndLifecycleIntegrationTest.testInventoryConcurrentCompletionSingleWinner` (二重確定1件)<br>`AssetBoundaryAndLifecycleIntegrationTest.testProviderRecoveryAndIdempotency` (PENDING/UNKNOWNから確認、同一key再送なし、別key拒否)<br>`AssetBoundaryAndLifecycleIntegrationTest.testOffboardingBlockersUseStatusOrDateContract` (NF-01 blocker OR条件)<br>`AssetBoundaryAndLifecycleIntegrationTest.testSoftDeleteInvariants` (終端履歴削除拒否)<br>`AssetMySqlIntegrationTest` の5並行テスト（claim、異なるaccountの同一key、返却/免除、license解放、棚卸し確定） | **PASS** |
...
5. **移管/返却/紛失/廃棄履歴を上書きしない (AS-R1.4)**:
   ...
   - `AssetLifecycleAppendOnlyApiContractTest` で貸与・外部account・event serviceから汎用 `IService` mutation入口を除去し、`AssetMySqlIntegrationTest.testV132AndV133SchemaAndAppendOnlyGuardsOnMySQL` でV132のevent UPDATE/DELETE trigger、waiver scope列・unique/FK、およびV133の列/unique/FKを確認する。終端履歴の論理削除も状態にかかわらず拒否し、`RETURNED`/`REVOKED`/`RELEASED`を保持する。
...
- **検証対象実装remote Head**: 第9回Review追加是正の実装・テスト・文書commitはpush後に、最終Review開始時点で `git rev-parse HEAD` と `git ls-remote origin refs/heads/codex/asset-account-license-lifecycle` が一致することを確認する。
...
| P1-04 MySQL競合未検証 | claim、返却/免除、license二重解放、棚卸し二重確定をMySQL Testcontainersへ追加 | `AssetMySqlIntegrationTest` 9/9、skip=0 |
...
| P2-03 V132 shape | waiverのscope列、unique/FK、event triggerをDDLに追加 | `AssetMySqlIntegrationTest.testV132AndV133SchemaAndAppendOnlyGuardsOnMySQL` |
...
| 指摘 | 対応 | 検証 |
|---|---|---|
| P1-01 資産6状態契約 | `AssetServiceImpl` の登録/状態変更で6値を検証し、`RESERVED` をDDLコメント、Entity、検索、一覧バッジ、棚卸し入力、状態機械へ同期 | `AssetServiceTest.testAssetFullLifecycle`、NF-09 Fast 72/72 |
| P1-02 返却日当日の再貸与 | 重複判定の実返却境界を `actual_return_date > startDate` に変更し、同日再貸与を許可 | `AssetBoundaryAndLifecycleIntegrationTest.testReassignImmediatelyAfterReturn` |
| P1-03 NF-01 blocker OR | 資産を `status='ACTIVE' OR actual_return_date IS NULL`、licenseを `status='ACTIVE' OR released_date IS NULL` で照合 | `AssetBoundaryAndLifecycleIntegrationTest.testOffboardingBlockersUseStatusOrDateContract` |
| P2-01 Fast証跡 | R7追加の `AssetLifecycleAppendOnlyApiContractTest` を提出コマンドへ追加し、実測値を72/72へ更新 | review-evidenceの同一コマンド 72/72 |
| P2-02 異なるaccountの同一key競合 | unique競合を業務409へ変換しprovider呼出しを1件に限定 | `AssetMySqlIntegrationTest.testConcurrentRevokeClaimSameKeyAcrossAccountsReturns409OnMySQL`、MySQL 9/9 |

R8の対象gateはすべて0 failure / 0 error / 0 skipped。リポジトリ全体Fast gateはBase由来の残存失敗を含むため、未PASSのままPRは作成していない。

## 7. 第9回Review指摘への対応

| 指摘 | 対応 | 検証 |
|---|---|---|
| P1-01 資産状態遷移表未実装 | `AssetStatusPolicy` に6状態と許可遷移を集約し、`AssetServiceImpl` の状態変更を表3の遷移とCASで制約。`AssetService` から汎用 `IService` mutation APIを外し、`IN_STOCK ↔ ASSIGNED` は `AssetAssignmentService` 専用入口に限定 | `AssetServiceTest.testAssetStatusTransitionGuard`、`AssetLifecycleAppendOnlyApiContractTest`、NF-09 Fast 77/77 |
| P1-02 実返却日の期間不変条件 | 資産→貸与の固定lock内で `start_date <= actual_return_date <= 今日` を検証し、開始日前・未来日の返却を拒否。同日返却・同日再貸与は維持 | `AssetBoundaryAndLifecycleIntegrationTest.testActualReturnDateRangeIsEnforced` |
| P2-01 棚卸し6状態同期 | 棚卸しUIに `DISPOSED` を追加し、`AssetInventoryServiceImpl` で `observedStatus` をtrim・大文字化して6値検証 | `AssetServiceTest.testInventoryObservedStatusVocabulary` |
| P2-02 MySQL証跡件数 | R9追加テストをFast提出コマンドへ含め、現行値をFast 77/77へ更新。MySQL 9/9、V133 migration smoke 4/4、退社gate drill 9/9、shard inventory 1/1も同じHeadで再実行 | review-evidence/tasksと実測Surefire結果の一致 |

R9の対象gateはすべて0 failure / 0 error / 0 skipped。リポジトリ全体Fast gateはBase由来の残存失敗を含むため、未PASSのままPRは作成していない。

## 8. 第9回Review追加指摘への対応

| 指摘 | 対応 | 検証 |
|---|---|---|
| P1-01 AS-R3.3紛失インシデント追跡のPlan/実装欠落 | V133 `t_asset_lost_incident` と専用Entity/Service/APIを追加し、起票日時、報告者、リモートワイプ要求/実施/確認、警察届出、保険申請、関連DocumentLinkを永続化。LOST遷移と緊急通知outboxを同一transactionに統合し、インシデントIDでdedupe | `AssetServiceTest.testLostIncidentLedgerAndEmergencyAlert`、`AssetApiControllerTest.testLostIncidentApiFlow`、`AssetMySqlIntegrationTest.testV132AndV133SchemaAndAppendOnlyGuardsOnMySQL` のV133 shape assert |
| P1-02 LOST/DISPOSED専用処理の迂回 | `AssetService.changeStatus` の専用遷移先を拒否し、新規登録をIN_STOCK固定。`reportLost` はincident/通知付き、`disposeAsset` はDISPOSED event付きの専用入口とし、要員controllerの個別通知呼出しを除去 | `AssetServiceTest.testAssetStatusTransitionGuard`、`AssetServiceTest.testLostIncidentLedgerAndEmergencyAlert`、`AssetApiControllerTest.testLostIncidentApiFlow` |
