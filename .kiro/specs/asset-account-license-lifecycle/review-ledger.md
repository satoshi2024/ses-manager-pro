...
- **Handoff Payload Commit**: `(Pending latest commit on remote push)`
...
| **AS-R2** | 外部アカウント参照・秘密非保存原則・失効確認分離・Recovery/Idempotency・ライセンス席数CAS（`-1 / = / +1`） | `m_external_account_system`, `t_external_account_reference`, `m_license_plan`, `t_license_assignment`, `ExternalAccountService`, `LicenseService` | `AssetComprehensiveSecretScanTest`<br>`AssetSecretFieldScanTest`<br>`AssetBoundaryAndLifecycleIntegrationTest.testLicenseSeatLimitBoundaryMinusOneEqualPlusOneAndReassign`<br>`AssetBoundaryAndLifecycleIntegrationTest.testProviderRecoveryAndIdempotency`<br>`AssetOffboardingServiceTest.testProviderRevokeConfirmationTimeout` | **PASS** |
...
| **AS-R4** | 認可スコープ（全方位一致）・要員マイポータル・期限通知（7/3/当日/週次/リース満了）・監査ログ | `AssetScopeService`, `MyAssetApiController`, `AssetAlertService`, `AssetLifecycleScheduler`, `templates/my/assets.html` | `MyAssetApiControllerTest.testMyAssetPortalFlow`<br>`AssetAlertServiceTest.testCheckOverdueAssignments`<br>`AssetAlertServiceTest.testCheckExpiringLeases`<br>`AssetApiControllerTest` | **PASS** |
| **CR-01** | 認証・認可 (Spring Security / ActionPermissionResolver / DataScope 一貫適用) | `SecurityConfig`, `ActionPermissionResolver`, `AssetScopeService` | `AssetApiControllerTest`, `MyAssetApiControllerTest` | **PASS** |
| **CR-02** | 状態・競合・冪等性 (CAS / FOR UPDATE 排他 / 二重失効安全) | `AssetAssignmentServiceImpl`, `LicenseServiceImpl`, `ExternalAccountServiceImpl` | `AssetAssignmentConcurrencyTest` (並行4スレッド排他実証)<br>`AssetBoundaryAndLifecycleIntegrationTest.testProviderRecoveryAndIdempotency` | **PASS** |
...
| **CR-04** | 監査・セキュリティ・包括的秘密非保存 (Comprehensive No-Secrets Policy) | `AssetComprehensiveSecretScanTest`, `ApiAuditFilter`, `t_asset_event` | `AssetComprehensiveSecretScanTest` (DDL・Entity・DTO・UI 0 secret fields 検証) | **PASS** |
...
| **CR-06** | テスト完全性 (Fast H2 / 並行性 / 退社ゲート / 境界 / スキャン スキップ0件) | 全 10 テストクラス (28 メソッド) | `mvn test` (**28/28 tests PASS, 0 failure, 0 error, 0 skipped**) | **PASS** |
...
2. **password/token/recovery code用column/DTO/logを作らない (AS-R2.3 / CR-04)**:
   - `AssetComprehensiveSecretScanTest`: DDL、Entity/DTO、HTML input、JS payload の全方位スキャンで 0 違反を検証。
3. **external revoke requestとconfirmed resultを区別し、timeoutを成功扱いにしない (AS-R2.4)**:
   - `AssetBoundaryAndLifecycleIntegrationTest.testProviderRecoveryAndIdempotency`: FAILED_OR_TIMEOUT 時に SUSPENDED/PENDING_CONFIRMATION を維持し、プロバイダ復旧後に確認完了、二重失効確認は安全に冪等処理。
