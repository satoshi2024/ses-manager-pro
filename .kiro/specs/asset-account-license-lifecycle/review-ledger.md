...
| **CR-06** | NF-09対象テストの完全性（専用Fast 75件 + MySQL実コンテナ 9件） | NF-09対象15テストクラス (75メソッド) + MySQL 1クラス (9メソッド) | NF-09専用Fast: **75/75 tests PASS (0 failure, 0 error, 0 skipped)**<br>MySQL Gate: `mvn test -Pmysql-tests -Dtest=AssetMySqlIntegrationTest` (**9/9 PASS, 0 failure, 0 error, 0 skipped**)。V132追随の既存migration smoke 4件、実退社gate drill 9件もPASS。`MySqlTestShardInventoryTest` PASS & `AssetMySqlIntegrationTest` 登録。固定seed `27838638095700` の全体比較は Base `3060 tests / 2 failures / 16 errors / 0 skipped`、R6実装時点 `3123 tests / 2 failures / 11 errors / 0 skipped`。残存はBaseにも存在する既存/環境側テスト。全体Fast gateは **未PASS** | **NF-09対象PASS / 全体gate未PASS（Base比較済み）** |
...
| 指摘 | 対応 | 検証 |
|---|---|---|
| P1-01 資産状態遷移表未実装 | `AssetStatusPolicy` に6状態と許可遷移を集約し、`AssetServiceImpl` の状態変更を表3の遷移とCASで制約。`AssetService` から汎用 `IService` mutation APIを外し、`IN_STOCK ↔ ASSIGNED` は `AssetAssignmentService` 専用入口に限定 | `AssetServiceTest.testAssetStatusTransitionGuard`、`AssetLifecycleAppendOnlyApiContractTest`、NF-09 Fast 75/75 |
...
| P2-02 MySQL証跡件数 | R9追加テストをFast提出コマンドへ含め、現行値をFast 75/75へ更新。MySQL 9/9、migration smoke 4/4、退社gate drill 9/9、shard inventory 1/1も同じHeadで再実行 | review-evidence/tasksと実測Surefire結果の一致 |
