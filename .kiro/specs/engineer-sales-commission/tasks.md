...
- [x] A. 要員担当 UI 【並行可・担当ファイル: engineer/*.html, engineer*.js, EngineerApiController, dto/engineer/】
...
- [x] B. 契約帰属 【並行可・担当ファイル: Contract*(Impl/Renewal/Mapper/ListDto/ApiController), contract/list.html, contract.js, ContractServiceImplTest】
...
- [x] C. 営業成績サービス+画面 【並行可・担当ファイル: SalesPerformance*, dto/salesperformance/, templates/sales-performance/, sales-performance.js, sidebar.html】
  ...
  - **実装メモ**: 2026-07-16 レビューで、対象月稼動判定の `startDate=NULL` 耐性、成約件数/成約率の口径注記、
    論理削除済み営業ユーザー名の保持(`SysUserMapper.selectByIdsIncludingDeleted`)を補強。
    `SalesPerformanceServiceImplTest` は 8 件(営業ユーザー表示、過去営業、論理削除済み営業名、
    成約率百分率、work_record 優先、契約別上書き、更新契約除外、マイナス粗利0円、開始日NULL)を確認。
    `MessageBundleConsistencyTest` で i18n 4ファイルのキー整合も確認。

- [x] D. 待機一覧強化 【並行可・担当ファイル: AnalyticsServiceImpl, BenchEngineerDto, analytics/index.html, analytics.js, AnalyticsServiceImplTest】
