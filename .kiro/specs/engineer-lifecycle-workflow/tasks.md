...
- [ ] A2. `/my/lifecycle` 要員本人画面と公開フィールド境界
  - **Objective**: 要員本人がセルフサービスポータルから自分宛のライフサイクルタスク（`is_engineer_visible = 1`）を閲覧し、自己完了・証跡提出を行える画面（`/my/lifecycle`）を提供する。
  - **実装ガイダンス**: `/api/my/lifecycle/**` 専用エンドポイント。`is_engineer_visible = 0` の内部タスクはSQLレベルで完全除外。
    他人の案件へのアクセス試行に対しては 404 Not Found を返却（IDOR対策）。
  - **テスト要件**: L2〜L3。本人A/本人Bのスコープ隔離テスト、内部タスクが本人APIレスポンスに1件も漏洩しないことの検証、自己完了・証跡アップロード。
  - **Demo**: 要員アカウントでログインし、自分宛の公開タスクのみが表示され、内部セキュリティタスクが表示されないことを確認。
