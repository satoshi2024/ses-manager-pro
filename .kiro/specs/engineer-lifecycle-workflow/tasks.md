...
- [x] B2. 証跡・文書台帳連携・運用手順・補償
  - **Objective**: タスク完了時の証跡として法定文書台帳（`DocumentService` / `t_document` / `t_lifecycle_task_evidence`）との原子的な紐付けを行い、案件取消時の補償処理および運用ランブックを整備する。
  - **実装ガイダンス**: `DOCUMENT_LINK` 証跡における実体存在確認、案件取消時（`cancelCase`）の補償トランザクション処理（関連タスクの無効化・監査イベント記録・通知）。
  - **テスト要件**: L1〜L4。証跡紐付け整合性、案件取消時の補償処理・二重完了防止、例外ロールバック検証。
  - **Demo**: 存在しない文書IDの紐付けが拒否されること、案件取消時に全タスクがCANCELLEDとなり再開・完了が禁止されることを確認。

- [x] M. 統合ゲート・退社障害訓練・独立Review引き渡し
  - **Objective**: fast test、MySQL実コンテナ（Flyway smoke）、および退社障害訓練（セッション失効・アカウントロック漏れ防止）を含む全テストゲートを実行し、スキップ0件を検証して独立Reviewへ引き渡す。
  - **実装ガイダンス**:
    1. 退社障害訓練テスト (`ResignationGateFailureDrillTest`): 8項目ゲート検証、未返却資産・未精算経費・阻害タスク未完了時の完了ブロック、全タスク完了後の自動アカウント無効化・セッション強制失効・主担当営業解除・組織所属閉鎖を検証。
    2. 全テストスイート実行（2688件全パス、0失敗、0スキップ）。
    3. 実MySQL/Flywayテスト (`LifecycleMySqlIntegrationTest`) の追加および `mysql-shard-1.txt` への登録。
  - **テスト要件**: L1〜L6。全2,688件のJUnitテスト実行、全ゲートPASS、スキップ0件。
  - **Demo**: `mvn test` で全テストがPASSし、スキップが0件であることを確認。合格証拠、退社時アクセス遮断シミュレーションのログ、および変更ファイル一覧を報告。
