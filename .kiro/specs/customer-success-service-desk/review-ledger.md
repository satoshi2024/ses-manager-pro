...
| F1 | CS-R1/R2/R3 DDL | `118a1046` | commit後更新 | V147、H2 schema、entity/mapper、migration tests | fresh/V144→V147/旧V110 MySQL + smoke | migration | 最終gate実測 | runbook reset | NO |
...
| `WIP-2` | High | `m_service_sla_policy` の `uk_sla_policy_priority` を `idx_sla_policy_priority`（INDEX）に変更し、版管理衝突を解消。現行NF-02統合DDLはV147。 | `V147__customer_success_service_desk.sql`, `schema-service-desk-h2.sql` |
...
| `WIP-10` | High | `V1__create_tables.sql` から旧NF-02 V110由来の CREATE / DROP を完全削除し、baseline 規約に準拠。現行統合版はV147であり、旧V110はreset/repair fixtureのみで扱う。 | `V1__create_tables.sql`, `V147__customer_success_service_desk.sql` |
| `WIP-11` | High | コメント読取の SQL `visibility='PORTAL_VISIBLE'` 保証、keyword 検索で INTERNAL コメント探索を完全除外。 | `ServiceRequestServiceImpl.java` |
| `[P0] NF02-MAIN-P0-01` | P0 | V147のポータル組織列を実在する `m_portal_organization.type` へ修正。upgrade fixtureも実在する `m_customer` を使用し、fresh/V144→V147/旧V110 repairを分離検証。mainのV144 digital_invoice・migration-dev V145/V146を保持。 | `V147__customer_success_service_desk.sql`, `FlywayCustomerSuccessServiceDeskUpgradeTest`（MySQL gate） |
...
- remote Head: rebase後commit（Base `origin/main@118a1046`、migration **V147**）。
...
- Review FAIL Head `6fc44e15` を `origin/main@118a1046` へrebase。NF-02 migrationは **V147**（main V144 `digital_invoice_safe_diagnostics` + migration-dev V145/V146 を保持）。`o.type = 'CUSTOMER'` 維持。upgrade fixture は `m_customer`。
- NF-02定向 H2（8クラス）+ `MigrationScriptIntegrityTest` + `FlywayMigrationVersionResolutionTest` + `MySqlTestShardInventoryTest`: **`107 / 0 / 0 / 0`**（V144=digital_invoice、V147=customer_success、重複なし）。
- 実MySQL定向（clean後）: Upgrade 3（fresh→V147、V144→V147、旧V110 reset）+ SchemaSmoke 1 + Concurrency 3 — **`7 / 0 / 0 / 0`**。
- mysql shard gate:
  - shard-1: **`35 / 0 / 0 / 0`**
  - shard-2: **`45 / 0 / 0 / 0`**
  - shard-3: **`36 / 0 / 0 / 0`**
  - 合計 **`116 / 0 / 0 / 0`**
- fast gate (`mvn test`): **1 failure** = `AssetBoundaryAndLifecycleIntegrationTest.testSystemVsManualActorAttribution`（単体再実行 PASS。NF-09 順序依存flake。NF-02変更外）。
- `git merge-tree origin/main HEAD`: CONFLICT マーカーなし。`db/migration` に V144×1（digital_invoice）・V147×1（NF-02）のみ。
