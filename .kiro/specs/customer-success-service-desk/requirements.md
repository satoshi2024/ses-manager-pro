...
**承認状態**: 2026-08-27 時点で traceability は CANDIDATE/DISCOVERY。Owner・Approved scope・DecisionId・DG-02 は未APPROVED。本ブランチの実装はユーザー指示による統合hardeningであり、production releaseの承認を意味しない。
...
4. THE status SHALL `RECEIVED → IN_PROGRESS → WAITING_CUSTOMER → RESOLVED → CLOSED` の許可行列だけを受け付ける。HTTP状態変更commandの`version`は必須とし、`RESOLVED`/`CLOSED` からの再openは新SLA roundを作り、以前のSLA結果を上書きしない。永続statusに曖昧な `REOPENED` を残さず、遷移コマンド `REOPENED` は新round作成後 `IN_PROGRESS` へ進める。不許可遷移、stale version、affected rows=0 は409とし、状態イベントを記録しない。
...
6. THE 既存請求メモ `t_invoice.portal_inquiry` SHALL 本機能のrequestへ自動変換しない。両方を併存する。作成・復帰・再openはtenant、ZoneId、Instant、organization/legalEntityを明示した実行コンテキストで処理する。
...
4. THE scheduler SHALL ShedLockで多重起動を1実行に畳み、期限前warning・初回breach・継続breachを `NotificationService.publishToUser` で通知する。受信者なし/配信失敗はescalation台帳へ保存し、退避後に同一dedupe keyで冪等再試行する。dedupe keyは request×round×種別×段階で一意とし、同一事象の再実行で通知を増やさない。
...
1. THE health SHALL 未解決P0件数、未解決P1件数、直近30日SLA違反件数、**直近90日**の平均CSAT、AR延滞（既存Invoice overdueの読取）、最終QBR日を型付きfactorとして計算する。合計0–100と `HEALTHY`(≥80)/`WARNING`(50–79)/`CRITICAL`(≤49) を返す。
...
4. THE 日次snapshot SHALL customer×日付×versionで一意とし、同一日の再計算で過去snapshotをUPDATE/DELETEしない。データ変更による訂正は非空理由付きのINSERT専用revisionとし、最新版は最大versionで解決する。current時点以外のtargetMonthはas-of算定を提供するか、未提供なら拒否する。
...
2. THE 状態変更 SHALL `version` と旧statusを条件にしたCASで防重し、affected rows=0は409とする。CAS失敗後に状態イベントを作成してはならない。
...
6. THE 各acceptance criterion SHALL 定向自動testまたは明示Demoへtraceする。skipを成功としない。snapshot serviceのSecurityContextなし呼出しはデフォルト拒否し、管理者または厳格なSYSTEM schedulerコンテキストだけを許可する。
