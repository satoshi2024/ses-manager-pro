...
- 状態: `READY_FOR_REVIEW`
- 実装AI: S14主実装（Round 4 Review Packet再構成）
- 独立Review: Round 4 提出（P1 12件・P2 2件の全修正を `FIXED_BY_IMPLEMENTER` として記録・独立検証待ち）
...
| **S14 Tests** | `src/test/java/com/ses/migration/FlywaySelfServiceSchemaSmokeTest.java` | V105.1旧適用済みDB→V105.2順方向適用fixture追加 |
| **中央台帳** | `.kiro/specs/customer-product-expansion-2026/spec-execution-ledger.md` | S14進捗行同期 |
...
| **R1-P1-05** | P1 | 会計連携ジョブ送信通知のtx内永続化 | `notifyAccountingSent` を `markSent` トランザクション内に包含し、ジョブ更新と通知を原子的にコミット | `ExpenseRequestFlowIntegrationTest` | `FIXED_BY_IMPLEMENTER` |
...
| **R1-P1-09** | P1 | 1on1 日程確定・キャンセルの状態機械と境界 | 確定前・確定後・実施済の各状態遷移および過去日・無効要員のバリデーションを網羅 | `OneOnOneSurveyFlowIntegrationTest` | `FIXED_BY_IMPLEMENTER` |
...
| **R1-P1-11** | P1 | V105.1 blobチェックサム復元とV105.2新設分離 | `V105_1` を `4fa3a689` 元 blob へ完全復元。追加列（`phone` / `template_snapshot_version`）は新設 migration `V105_2` へ分離 | `FlywaySelfServiceSchemaSmokeTest.旧4fa3a689版V105_1適用済みDBからV105_2へ順方向適用できる` | `FIXED_BY_IMPLEMENTER` |
...
| **R1-P2-03** | P2 | テナント Clock 注入の徹底 | `OneOnOneRequestServiceImpl` および `EngineerChangeRequestServiceImpl` から引数なし `.now()` を完全排除し `LocalDate.now(clock)` / `LocalDateTime.now(clock)` で統一 | 全結合テスト（Clockモック互換） | `FIXED_BY_IMPLEMENTER` |

## Row-Level Test Matrix（決定表・要件対応テスト一覧）

| 要件ID | 決定表項目 | 実装クラス / メソッド | 正常系テスト | 異常系・拒否テスト | 境界値・競合テスト |
|---|---|---|---|---|---|
| **R1.1** | プロフィール申請項目 (phone, station等) | `EngineerChangeRequestServiceImpl.createDraft` / `myProfile` | `電話番号変更申請が承認後に反映される` | `allowlist外項目の申請を拒否する` | `myProfileで未登録時null返却` |
| **R1.2** | 理由・添付・承認反映 | `MyChangeRequestApiController.uploadAttachment` / `validateAttachment` | `添付ファイル付き変更申請の一気通貫フロー` | `未所有または未スキャン添付ファイルの指定を拒否する` | `CLEANスキャンのみ許可` |
| **R1.3** | スキルシート公開確認日 | `MyProfileApiController.confirmSkillSheet` | `スキルシート確認日記録テスト` | `未紐付けアカウントの確認拒否` | `version一致確認` |
| **R1.4** | 担当営業・契約条件表示（原価非公開） | `MyProfileApiController.myDashboard` | `本人ダッシュボード取得テスト` | `原価・粗利フィールド非公開検証` | `契約終了日NULL安全表示` |
| **R2.1** | 本人給与明細閲覧 | `MyPayrollApiController.getStatement` / `FreeeIntegrationServiceImpl.statementForEngineer` | `statementForEngineerの要員抽出と混入防止` | `未紐付け要員のアクセス拒否` | `他要員PII完全除外` |
| **R2.2** | 給与明細一覧非露出・再認証 | `MyPayrollApiController.reauthenticate` | `再認証後詳細取得テスト` | `再認証なし詳細アクセス403拒否` | `10分有効期限切れ後再拒否` |
| **R3.1** | 経費申請・領収書添付 | `MyExpenseApiController.create` / `validateExpense` | `経費申請作成と領収書紐付け` | `不正区分・金額0円拒否` | `上限金額境界値` |
| **R3.2** | 経費状態遷移 | `ExpenseRequestServiceImpl.submit` / `ExpenseApprovalAdapter` | `経費一気通貫フロー` | `承認後領収書差替え拒否` | `差戻し後の再申請` |
| **R3.4** | 会計連携冪等性 | `ExpenseAccountingJobScheduler.dispatchOne` | `会計連携ジョブ送信とtx内通知` | `送信失敗時リトライ可能` | `job UNIQUEキー重畳防止` |
| **R4.1** | 1on1候補日・相手方検証 | `OneOnOneRequestServiceImpl.create` | `1on1作成と日程確定フロー` | `無効マネージャー指定400拒否` | `過去日付候補日400拒否` |
| **R4.2** | サーベイ定期回答 | `SurveyServiceImpl.submitAnswers` | `サーベイ回答送信テスト` | `同意なし回答拒否` | `期間開始日・終了日当日回答可` |
| **R4.3** | confidential相談閲覧制限 | `OneOnOneRequestServiceImpl.detailManagement` / `AuthorizationServiceImpl` | `confidential秘密メモはHRのみまたは指定管理者のみ閲覧可能` | `一般管理者・マネージャー不可視` | `privateNoteRef=nullマスク` |
| **R4.4** | サーベイ離職リスク・匿名性閾値 | `SurveyServiceImpl.aggregate` / `computeRetentionRisk` | `サーベイ集計正常系` | `匿名性閾値未満の質問別リスク分析は集計から除外される` | `回答数2件/閾値3件時hidden=true` |
| **R5** | 本人A/B完全分離・master不変 | 各MyApiController / ApprovalAdapter | `本人AB間データ分離統合テスト` | `IDORアクセス404/403拒否` | `SysUser並行変更時409 conflict` |

## Migration Fixture 実証記録

| 検証シナリオ | 対象環境 / ツール | 検証内容 | 結果 |
|---|---|---|---|
| **Fresh DB** | MySQL 8.0 (Testcontainers) / Flyway | V1統合baseline → V105.2順方向適用 | PASS (テーブル・列・制約・seed整合) |
| **Legacy DB (V104.4)** | MySQL 8.0 (Testcontainers) / Flyway | V104.4適用済みDB → V105.2順方向適用 | PASS (selfserviceテーブル追加・shape一致) |
| **Old V105.1 DB (4fa3a689)** | MySQL 8.0 (Testcontainers) / Flyway | V105.1適用済みDB → V105.2順方向適用 | PASS (`phone`, `template_snapshot_version` 追加) |
...
- **Active Profile**: `test`
- **結果**: **Tests run: 52, Failures: 0, Errors: 0, Skipped: 0**
...
- **結果**: **Tests run: 2318, Failures: 0, Errors: 0, Skipped: 0**
- **Exit code**: `0` (BUILD SUCCESS)
