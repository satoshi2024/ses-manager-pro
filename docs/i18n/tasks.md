...
- [ ] **T2. フロントJS辞書機構 + common.js**(T1 完了後)
  - 対象: `common/i18n/I18nMessagesLoader.java`、`controller/page/I18nJsController.java`(`/js/i18n.js`)、`base.html`/`login.html` への読込追加(common.js より前・**同期ロード**)、`common.js`(`SES.i18n.t/e` + 全文言キー化 + Intl 日時)
  - **★必須**: `I18nMessagesLoader` は `.properties` を **UTF-8 明示**で読む(`EncodedResource(resource, UTF-8)`)。ISO-8859-1 既定のまま読むと多言語が文字化けする(plan.md 決定4)
  - テスト: `I18nJsControllerTest` 新設(辞書に文字化けが無いこと=日本語キーの既知値が正しく載ることも確認)
...
- [ ] **P-E. Java層メッセージ**(テンプレート/JSに触れないため P-A〜D と競合しない)
  ...
  - **★必須**: `getValidator()` では `bean.afterPropertiesSet()` を呼ぶ(未初期化だと全バリデーションが例外・plan.md 決定6)。`WebConfig` は T1 で凍結済みのため、P-E で触るのは `getValidator()` 追加のみ(localeResolver 等には触れない)
